...
if   command -v pacman        >/dev/null 2>&1; then
    ...
    # NO lua-cjson HERE. Arch does not ship it in a repository -- it is AUR-only
    # -- and pacman fails the WHOLE batch on one name it does not know, so
    # including it took the toolchain down with it. Handled below instead, where
    # having the module is tested for directly rather than assumed from a name.
    pkgs="cmake gcc qt6-base qt6-declarative layer-shell-qt lua"
...
# --- lua-cjson, by asking Lua rather than the package manager -------------
#
# The engine opens with `require("cjson")`, so without this module spoot does not
# start -- and it is the one dependency whose PACKAGE NAME is unreliable
# everywhere: Debian and Fedora ship it, Arch keeps it in the AUR, and the module
# has to match the interpreter's ABI version besides.
#
# So the test is whether Lua can load it, which is the only question that
# actually matters, and the remedies are tried in order of how little they ask of
# you. An AUR helper is never run through sudo -- makepkg refuses to build as
# root, and that refusal is correct.
have_cjson() { command -v lua >/dev/null 2>&1 && lua -e 'require("cjson")' >/dev/null 2>&1; }

if ! have_cjson; then
    say "installing lua-cjson"
    if   command -v paru >/dev/null 2>&1; then paru -S --needed --noconfirm lua-cjson || true
    elif command -v yay  >/dev/null 2>&1; then yay  -S --needed --noconfirm lua-cjson || true
    elif command -v luarocks >/dev/null 2>&1; then ${priv}luarocks install lua-cjson || true
    fi
fi
if ! have_cjson; then
    warn "lua cannot load cjson, and spoot's engine will not start without it."
    warn "install it with your distribution's lua-cjson package, an AUR helper"
    warn "on Arch, or: luarocks install lua-cjson"
    warn "carrying on with the build -- everything else here is unaffected."
fi
...
# download -- or was copied from another machine -- makes cmake refuse before it
# does anything, naming a path that means nothing on this computer. Removed
# rather than reported: it is a cache, it costs seconds, and nobody downloading
# spoot wants to be told to delete a directory they did not create.
#
# A cache that belongs here is left alone, so an incremental rebuild stays
# incremental.
