...
Window {
    ...
    // TWO BORDERS, NOT ONE. The column is inset by the border on every side
    // (see its anchors.margins), so it has the panel's height less a border at
    // the top AND one at the bottom to fill. Counting one -- a leftover from when
    // the panel sat flush on the screen edge, where the bottom border was never
    // drawn -- left the content a pixel taller than its box, and a Column does not
    // clip: the body's last row, which is where a backdrop sits, painted over the
    // bottom border and past the rounded corner the foot mask cuts for it.
    readonly property int menuHeight: message.height + bodyHeight
                                     + noticeBar.height + zenon.borderWidth * 2
    ...
    // IS THIS HOP A CARD. Both kinds -- and it knew only one, which is the whole
    // of "cards still persist in the stack".
    //
    // A card arrives on the trail two ways. Shift+Return sends a STEP carrying
    // `alt`, and that has been recognised here since the day skipCtx was written.
    // app.openCard sends a ROOT hop instead (Alt+E, Alt+F, Alt+Return, the
    // backdrop, the listener's result), and this did not know about those at all
    // -- so the moment one of them led somewhere real and applyWhere adopted the
    // chain, the card became an ordinary trail step you could stand on. Backspace
    // or Alt+left then walked ONTO it, the engine replayed the segment, the view
    // answered `context`, and the card you had closed opened itself again out of
    // the trail. That is the thing coming back from the dead.
    //
    // `buf` is the mark openCard puts on its hop, and the engine reads the same
    // flag when it builds the crumb -- one word, both sides, so a card cannot be a
    // place in one of them and not the other.
    function isCtxHop(h) {
        if (!h) return false
        if (h.buf === true) return true
        return !!(h.step && typeof h.step === "object" && h.step.alt === true)
    }
    ...
    Connections {
        ...
        function onEvent(name, data) {
            ...
            // A MENU FROM TRAIL HISTORY, reopened as a hop of its own that carries
            // the path to it -- so Backspace can walk that path back up rather than
            // falling straight through to wherever Trail History was opened. The
            // card it was picked in goes with the push. See goBack.
            // Recorded, not applied: the pick that raised this answers with an
            // EMPTY draw right behind it, and an empty draw pops the last hop (see
            // render) -- which would be the hop pushed here. So the empty draw is
            // what applies it. The engine writes the event first, so it is always
            // waiting by then.
            else if (name === "restore") root.pendingRestore = data.stack || []
            ...
            // A LYRICS LOOKUP LANDED for the track an open action card is about.
            // The card drew before the answer existed -- an unknown state shows
            // the Lyrics row -- so it is redrawn in place now, and a track with
    ...
    function render(d) {
        ...
        if (d && d.empty === true) {
            ...
            // A TRAIL HISTORY PICK. Its reply is this empty draw, and what it
            // asked for is a place: the stack it carried becomes a `restore` hop
            // on the trail (see goBack, which walks it back up) instead of the
            // pop an empty draw otherwise means.
            if (root.pendingRestore) {
                var st = root.pendingRestore
                root.pendingRestore = null
                root.preHops = []
                root.prePos = -1
                root.pushHop({cmd: "view", key: "restore", restore: st})
                root.refresh()
                return
            }
    ...
    function patchRows(model, list, filter) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            var mt = r.meta || ""
            if (m.meta !== mt) model.setProperty(i, "meta", mt)
            var mk = r.marks || ""
            if (m.marks !== mk) model.setProperty(i, "marks", mk)
        }
        return true
    }
    ...
    function applyWhere(d) {
        ...
        if (root.ctxHops.length) {
            root.preHops = root.hops
            root.prePos = root.trailPos
            var joined = root.activeHops().concat(root.ctxHops)
            // A BUFFER THAT ENDS THE TRAIL IS NOT A BUFFER. Everything that runs
            // here is answering a REAL menu, and the last hop is what produced it
            // -- so if that hop is still marked as a card, the mark was wrong: the
            // view it names drew a place. Left marked it would be skipped by every
            // walk and, being the tip, skipped INTO nothing.
            //
            // Only the last one. The hops before it were genuinely walked THROUGH
            // to get here and stay marked, which is what keeps them out of the
            // crumb and out from under the cursor.
            var tip = joined.length ? joined[joined.length - 1] : null
            if (tip && tip.buf === true) {
                var settled = ({})
                for (var k in tip) if (k !== "buf") settled[k] = tip[k]
                joined[joined.length - 1] = settled
            }
            root.hops = joined
        ...
        if (d && d.keep !== undefined && !root.promptFor.length) {
            ...
            if (want < root.trailPos) {
                ...
                // ...AND NEVER TRIMMED BACK ONTO A CARD. `keep` counts steps
                // inside a segment and knows nothing about which of them were
                // taken through a buffer, so a trim can land exactly on one --
                // leaving a card as the tip, which the very next refresh would
                // draw. Walked back past it, the direction a trim travels.
                want = root.skipCtx(want, -1)
    ...
    function applyContext(d) {
        ...
        if (!root.ctxHops.length && root.hops.length
                && root.trailPos === root.hops.length) {
            // ...AND IT IS MARKED ON THE WAY OUT, which is the only moment
            // anything can know.
            //
            // openCard marks its hop `buf` in advance because it is asking for a
            // card on purpose. A plain Return cannot: Seek is row two of Playback
            // and the Search tile is row four of Main, and both look like any
            // other row until the answer comes back saying `context`. That answer
            // is this function, so this is where the fact exists.
            //
            // Unmarked, the hop went back onto the trail when applyWhere adopted
            // it and skipCtx had no way to tell it from a place -- so backing out
            // of search results landed on the Search hop, the engine replayed it
            // with an empty path, and the box opened itself again. That is the
            // card coming back, by the one route that was not covered.
            var was = root.hops[root.hops.length - 1]
            var mark = ({buf: true})
    ...
    function openCard(name) {
        // `buf` MARKS IT AS A BUFFER, and that mark has to survive being adopted.
        //
        // While the card is up this hop rides on ctxHops, which the engine walks
        // and then leaves out of the crumb -- so nothing is needed. But a step
        // taken INSIDE a card can land on a real menu (Go to Album, out of a
        // track's action menu), and applyWhere then adopts the whole chain onto
        // the trail so that place is walkable. The chain starts with THIS hop, and
        // on the trail it is a root: the crumb grew a seam and the card's own name
        // in the middle of a journey nobody took through it.
        //
        // A card is a buffer between places, not a place. The flag rides along and
        // Util.serve_nav honours it -- the segment is walked, and it contributes
        // no seam and no name of its own. What it LANDED on still does.
    ...
    function popTransient() {
        ...
        // Back past any card the dropped step was taken through: a buffer with
        // nothing beyond it is a card left standing at the end of the trail, and
        // that is the one position no walk may produce.
    ...
    Instantiator {
        ...
        delegate: Dock {
            ...
            // Reported up, so the playback poll knows a pill wants it. Dropped on
            // destruction too: a dock torn down while open (switched off, a
    ...
    //
    // ...AND ONLY WHILE THERE IS A DOCK TO FEED. The watch is a socket round trip
    // to Hyprland sixteen times a second, and it ran whenever the panel was away
    ...
    onSettingsChanged: root.syncCursorWatch()
    ...
    function jumpToCrumb(n) {
        ...
        // ...AND NEVER ONTO A CARD. trailMap is keyed by crumb LENGTH, and a card
        // contributes no crumb parts -- so the position it recorded carries the
        // same length as the step before it, and whichever wrote last wins. A
        // click on the trail could therefore land on a buffer and draw it. Walked
        // past in the direction of travel, exactly as trailBack and trailForward
        // already do.
        pos = root.skipCtx(pos, back ? -1 : 1)
    ...
    function jumpToPlaying() {
        ...
        if (root.originPos > 0 || root.originHops.length) {
            ...
            if (root.originOnTrail()) {
                ...
                // Never onto a card: the list a track was played from may have
                // been reached THROUGH one, and Alt+c is a walk like any other.
                root.trailPos = root.skipCtx(root.originPos, back ? -1 : 1)
    ...
    function tabHere() {
        ...
        if (root.ctxUp && root.ctxMode.length) {
            // `buf` FOR THE SAME REASON EVERY OTHER CARD CARRIES IT. This hop is
            // built here rather than through openCard, so it was the one card in
            // the app the general rule did not reach -- and the menu that LISTS
            // the trail is the last thing that should be able to end up in it.
            // There is a bespoke cleanup for exactly that a few hundred lines up
            // (it strips a `trail-jump` root by name after a jump); this is the
            // rule that makes the cleanup a second line of defence rather than
            // the only one.
            root.ctxHops = [{cmd: "view", key: "trail-jump", buf: true,
    ...
    function deleteEntry() {
        ...
        // ON WHICHEVER LIST HAS THE KEYBOARD, which is the whole of "delete on a
        // search history entry does nothing".
        //
        // This read focusModel and focusItem -- both of which follow a card the
        // moment one opens -- and then sent the step through pushHop, which is the
        // TRAIL. The search history is a card now (see isSearchPrompt), so the
        // press picked the right row out of the right list and then posted it to
        // the menu underneath, where that index means something else entirely or
        // nothing at all. The engine replayed it against the list behind the card
        // and the row you were looking at never moved.
        //
    ...
    function goBack() {
        ...
        if (root.ctxHops.length) {
            ...
            if (root.ctxHops.length) root.refresh(-1)
                ...
                var dirty = root.ctxDirty
                root.closeContext()
                // The list behind a used sticky card: redrawn once, in place.
                if (dirty) root.refresh(0)
            }
            return
        }
        root.rememberPos()
        ...
        // Trail History is one hop holding the whole stack that led to it, so
        // dropping the hop would leap past every menu on that path. While it
        // still has somewhere to go, it is replaced by the same hop one entry
        // shorter -- the engine replays that as the menu the step came from.
        var tipHop = root.trailPos > 0 ? root.hops[root.trailPos - 1] : null
        if (tipHop && tipHop.key === "restore" && tipHop.restore && tipHop.restore.length > 1) {
            var shorter = ({})
            for (var k in tipHop) shorter[k] = tipHop[k]
    ...
    Timer {
        ...
        // ONLY WHILE SOMETHING IS LOOKING.
        //
        // This ran unconditionally -- panel hidden, dock off, nobody on screen
        // reading a single one of the forty bindings that consume root.playback --
        // and it is not a cheap tick: each one is an engine round trip that reads
        // two files and makes a playerctl D-Bus call, and when nothing is playing
        // it lets get_playback out to the network as well.
        //
        // Util.playback_wait's own note names this as the thing that earns the
        // throttle: "spoot is resident, the UI polls once a second... 720 requests
        // an hour, ~17,000 a day, against the endpoint Spotify guards hardest, to
        // be told nothing every single time". The back-off it added bounds the
        // network half at one a minute; it cannot bound the half that should not
        // be running at all.
        //
        // The dock is the case that keeps it alive while the panel is down -- it
        // shows what is playing with spoot closed, so it needs this. With the dock
        // off and the panel hidden there is no reader, and `triggeredOnStart`
        // means the first tick lands the moment the panel comes back, so nothing
        // is stale when it matters.
        //
        // ...AND THE DOCK ONLY WHILE ITS PILL IS UP. Everything it draws from this
        // lives on the pill; a closed dock is a hot edge reading the cursor, not
        // the player. Gating on the dock SETTING kept this ticking all day behind
        // an edge nobody was touching. See Dock.qml's `engaged`, which starts it
        // at the touch so the pill opens on a fresh answer.
        running: root.opened || root.anyDockEngaged
        repeat: true; triggeredOnStart: true
        // ONE IN FLIGHT. The engine is a single worker, and while it is busy --
        // covers for a grid, a login waiting on the browser -- a tick every second
        // stacked up behind it and all of them ran back to back when it came
        // free, each one reading the player to say the same thing. A tick that
        // finds the last one unanswered simply skips; the next one asks.
        onTriggered: {
            if (root.pollId !== -1) return
            root.pollId = root.call("playback", {}, function (p) {
                root.pollId = -1
                root.playback = p || ({})
                root.syncPos(p)
    ...
    // The poll in flight, or -1. Cleared by its callback, and ALSO by onResponse
    // and by an engine restart -- a failed reply never reaches the callback, and
    // a guard that nothing clears would stop the poll for good.
    property int pollId: -1
    // The open card asked for the panel un-veiled (engine `unveil`). See panel.veil.
    property bool ctxUnveil: false
    // A Trail History pick waiting for its own (empty) reply. See the `restore` event.
    property var pendingRestore: null
    // A STICKY CARD THAT WAS USED. Its picks deliberately leave the list behind
    // alone (see applyContext), so the list is still showing what it showed
    // before -- "Opacity 80%" after picking 60. Closing the card is when that
    // stops being right, so goBack asks once more if this is set.
    property bool ctxDirty: false
    // Which docks are open or opening, by screen. A map rather than a count so a
    // dock reporting the same state twice, or being destroyed after reporting,
    // cannot leave it off by one.
    property var engagedDocks: ({})
    property bool anyDockEngaged: false
    function dockEngaged(name, on) {
        var m = root.engagedDocks
        if (on) m[name] = true
        else delete m[name]
        root.anyDockEngaged = Object.keys(m).length > 0
    }
    ...
    Component {
        ...
        TileGrid {
            ...
            //
            // Guarded on the same predicate `inert` reads rather than on dismissTop
            // answering false: this signal also fires at the end of an ORDINARY
            // click, and leaving it ungated works here only because nothing is in
            // front of this list while it is live. That coincidence does not hold
            // for the card's own list, where it closed the card on every press --
            // so the condition is written out in all three places.
            onRowClicked: if (gridBody.inert) root.dismissTop()
    ...
    Component {
        ...
        RowList {
            ...
            onPicked: function (i, alt) { if (!root.ctxUp) root.activate(i, alt) }
            // See gridView: a click behind a card closes it, the third button
            // queues -- and why the guard is on `anythingUp` rather than left to
            // dismissTop being a no-op.
            onRowClicked: if (listBody.inert) root.dismissTop()
            onRowQueued: function (i) { if (!root.ctxUp) root.queueRow(i) }
    ...
    Item {
        ...
        //
        // ...EXCEPT UNDER A CARD THAT IS PREVIEWING THE PANEL ITSELF: the Opacity
        // picker, whose whole subject is the ground this veil would cover.
        readonly property real veil: Math.max(root.ctxUnveil ? 0 : ctxLayer.opacity,
                                              sheetCard.opacity)
        ...
        Column {
            ...
            Item {
                ...
                Rectangle {
                    ...
                    // solid while the rest stays see-through.
                    color: "transparent"
                ...
                // THE PROGRESS, AND IT IS A LINE ALONG THE PANEL'S OWN TOP EDGE.
                //
                // It was a WASH: a horizontal gradient filling this whole bar,
                // solid grey to the playhead and feathered out after it. A fill is
                // a poor scale -- the edge of it is the only part that carries the
                // number, and that edge was deliberately softened over 34px so it
                // would not look cut, which is to say the one informative part was
                // the blurriest. It also sat behind the trail and the track name
                // and changed the contrast under them as it swept past.
                //
                // A line says the same thing and says it sharply, and there is a
                // natural place to run it: the top border of the panel, corners
                // included. It costs no height, it is nowhere near the text, and it
                // is the same idea the dock's pill already draws along its own
                // bottom edge.
                //
                // HOW IT GOES ROUND THE CORNERS. A Rectangle's border traces its
                // rounded outline, so the line is one -- drawn at the panel's own
                // radius, transparent in the middle, and REVEALED left to right by
                // a clip that grows with the track. The clip is only as tall as the
                // corner arc, so what shows is the top edge and the two curves and
                // nothing running down the sides.
                // ON THE BORDER, NOT INSIDE IT.
                //
                // This bar lives in a Column inset by zenon.borderWidth, so its own
                // origin is the panel's INNER corner -- a line drawn at 0,0 here
                // runs alongside the frame with a pixel of ground showing between
                // them, which reads as a second edge rather than as the edge
                // lighting up. So the whole assembly is placed in the PANEL's
                // coordinates instead: out by the border on both axes, wide by
                // twice it, and turning the panel's own outer radius. Nothing
                // between here and the panel clips (the Column does not, and the
                // panel is a plain Item), so the negative offsets land where they
                // are meant to.
                //
                // `over` is how far past the panel the glow is allowed to spill --
                // a glow that stops dead at the frame is a stroke, not a glow.
                readonly property int lineOut: zenon.borderWidth
                readonly property int lineOver: 3
                // The band the line and its spill occupy, in this bar's own
                // coordinates: it starts outside the panel's top-left and is wider
                // than the panel by the same amount at each end.
                readonly property real lineX: -(message.lineOut + message.lineOver)
                readonly property real lineW:
                    message.width + (message.lineOut + message.lineOver) * 2
                // HOW FAR THE SIDES CARRY ON PAST THE CORNER.
                //
                // The band used to stop at the bottom of the arc, so the line
                // turned the corner and was gone the moment it finished turning --
                // the verticals read as a hint of a corner rather than as the edge
                // lighting up and running down it. This is how much straight side
                // is allowed after the curve, and the fade below is stretched over
                // the same distance so what is added is a taper rather than a stub
                // with a blunt end.
                //
                // Off the radius rather than a flat number of pixels: it is a
                // proportion of the corner it continues from, so a theme with a
                // softer corner gets a correspondingly longer tail.
                readonly property int lineTail: Math.round(zenon.radius * 0.6)
                // Down to the bottom of the corner arc plus the tail and the spill,
                // so the two curves are complete and the sides run on a little.
                readonly property real lineH:
                    zenon.radius + message.lineOver + 6 + message.lineTail
                // How far the head dissolves over, as a fraction of that band --
                // fixed in PIXELS, so the softening is the same at the start of a
                // track as at the end rather than growing with the bar it has
                // filled.
                readonly property real lineSoft:
                    message.head <= 0 ? 0
                                      : Math.min(message.head, 34 / Math.max(1, message.lineW))
                // ...and at the end there is nothing beyond the head to dissolve
                // into: the line has arrived at the far corner and should close on
                // it, not fade out a moment before it gets there.
                readonly property bool lineDone: message.head >= 0.999
                // WHERE THE SIDES GIVE OUT. A rounded rectangle's outline does not
                // stop at the corner -- it turns and carries straight on down, and
                // the band below only decides where that gets CUT. Measured off a
                // render: full-strength green ran from the top of the arc to the
                // bottom of the capture and stopped flat, which is a vertical bar
                // down the panel's edge with a blunt end on it.
                //
                // So the line dissolves as it turns: full strength along the top
                // edge and into the first of the curve, gone by the time the arc
                // has finished turning -- which puts the end of it inside the
                // corner, where there is nothing left to cut.
                //
                // In band fractions, off the tokens that decide the shape rather
                // than as two numbers that happen to look right.
                readonly property real lineDropFrom:
                    (zenon.borderWidth + message.lineOver + 2) / message.lineH
                readonly property real lineDropTo:
                    (zenon.radius + message.lineOver + 1 + message.lineTail)
                    / message.lineH

                // THE STROKES, captured rather than drawn. Full width always -- the
                // MASK is what says how much of it has been played, so the shape
                // never changes as the track runs and the corner cannot crawl.
                // Same capture-mask-redraw shape the trail and the now-playing
                // title use.
                Item {
                    id: lineTrack
                    x: message.lineX; y: message.lineX
                    width: message.lineW; height: message.lineH
                    // Kept out of the scene by lineShot's hideSource and NOT by
                    // `visible: false` -- see the same note on crumbMask: an
                    // invisible item is not rendered at all, and the effect is then
                    // handed an empty texture.

                    // THE GLOW, which is the same line wider and turned down. A
                    // blur would be a second render pass on something that redraws
                    // every frame the playhead moves; two strokes is one draw call
                    // each and reads the same at this size.
                    //
                    // A Rectangle's border is drawn INSIDE its bounds, so this one
                    // is grown by `lineOver` all round and turns a correspondingly
                    // wider corner: its stroke then straddles the panel's frame,
                    // spilling outside it and washing a little way in.
                    Rectangle {
                        anchors { left: parent.left; top: parent.top }
                        width: message.lineW
                        // Tall enough that the BOTTOM edge is far below what is
                        // captured: what shows is the top edge and the two corners,
                        // never anything that reads as a box round the bar.
                        height: zenon.radius * 2 + 24
                        color: "transparent"
                        radius: zenon.radius + message.lineOver
                        border.width: 4
                        border.color: zenon.fade(zenon.playing, 0.22)
                    }
                    // ...AND THE LINE ITSELF, on the frame. Placed at the panel's
                    // own bounds with the panel's own radius, so its stroke covers
                    // the border pixel and one more inside it.
                    Rectangle {
                        x: message.lineOver; y: message.lineOver
                        width: message.width + message.lineOut * 2
                        height: zenon.radius * 2 + 24
                        color: "transparent"
                        radius: zenon.radius
                        border.width: 2
                ...
                ShaderEffectSource {
                    ...
                    x: message.lineX; y: message.lineX
                    width: message.lineW; height: message.lineH
                    sourceItem: lineTrack
                    hideSource: true
                    live: true
                    visible: false
                }
                // STAGE ONE: the sides give out.
                //
                // TWO PASSES BECAUSE A GRADIENT HAS ONE DIRECTION. What the mask
                // has to be is the playhead ACROSS multiplied by the drop-off
                // DOWN, and nothing in Qt Quick multiplies two gradients into one
                // alpha -- stacking them composites, which adds coverage rather
                // than taking it away. Masking twice is the multiplication: each
                // pass scales the alpha that reaches it.
                //
                // Down first, because it is the one that does not move. This pass
                // is identical on every frame of a track and only the second has
                // to follow the playhead.
                MultiEffect {
                    id: lineDrop
                    x: message.lineX; y: message.lineX
                    width: message.lineW; height: message.lineH
                ...
                ShaderEffectSource {
                    ...
                    width: message.lineW; height: message.lineH
                    sourceItem: lineDrop
                    // Takes the stage out of the scene, the same way lineShot does
                    // with the strokes -- and NOT `visible: false` on lineDrop
                    // itself, which would stop it being rendered and hand this an
                    // empty texture. See crumbMask.
                    hideSource: true
                    live: true
                    visible: false
                }
                Rectangle {
                    id: lineDropMask
                    width: Math.max(1, message.lineW)
                    height: Math.max(1, message.lineH)
                    gradient: Gradient {
                        // Spelled out, though it is the default: the mask beside
                        // this one says Horizontal, and the whole point of the pair
                        // is that they run in different directions.
                        orientation: Gradient.Vertical
                        GradientStop { position: 0.0; color: "white" }
                        GradientStop { position: message.lineDropFrom; color: "white" }
                        GradientStop { position: message.lineDropTo; color: Qt.rgba(1, 1, 1, 0) }
                        GradientStop { position: 1.0; color: Qt.rgba(1, 1, 1, 0) }
                    }
                }
                ShaderEffectSource {
                    id: lineDropMaskShot
                    sourceItem: lineDropMask
                    width: Math.max(1, message.lineW)
                    height: Math.max(1, message.lineH)
                    hideSource: true
                    live: true
                ...
                // STAGE TWO: the playhead, over what stage one left.
                MultiEffect {
                    x: message.lineX; y: message.lineX
                    width: message.lineW; height: message.lineH
                    ...
                    source: lineDropShot
                    maskEnabled: true
                    maskSource: lineMaskShot
                    // See crumbBar: at min 0 the ramp runs from -spread to 0 and
                    // every alpha lands at full strength, so the mask does nothing
                    // at all. At min 0.5 with spread 1 it is smoothstep(0, 1, a) --
                    // the whole alpha range, as a curve.
                    maskThresholdMin: 0.5
                    maskSpreadAtMin: 1.0
                    autoPaddingEnabled: false
                }
                // THE MASK, and it is the progress. Opaque to the playhead and
                // dissolving over the last 34px of it -- which is the one thing a
                // clip could not do: a scissor rectangle has a hard vertical edge,
                // and on a 1px line that edge is a cut end rather than a head.
                //
                // The old wash wore this exact gradient as its own colour. It is a
                // better mask than it was a fill: as a fill the softened part WAS
                // the reading, and here the reading is where the bright line stops.
                Rectangle {
                    id: lineMask
                    width: Math.max(1, message.lineW)
                    height: Math.max(1, message.lineH)
                    gradient: Gradient {
                        ...
                        GradientStop { position: 0.0; color: "white" }
                        GradientStop { position: Math.max(0, message.head - message.lineSoft)
                                       color: "white" }
                        GradientStop { position: Math.min(1, message.head)
                                       color: message.lineDone ? "white" : Qt.rgba(1, 1, 1, 0) }
                ...
                Item {
                    ...
                    NowContent {
                        ...
                        // THE MARKS AS FACTS, not as the engine's finished
                        // `icons` string -- see NowContent, where two of the three
                        // became controls and the third became the one coloured
                        // thing on the line. `icons` is still what the dock and
                        // every list row wear.
                        liked: root.playback.liked === true
                        explicit: root.playback.explicit === true
                        lyrics: root.playback.lyrics || "none"
                        ...
                        // The lyrics mark is a PLACE, so it navigates rather than
                        // going through control() with the transport. The same
                        // view Alt+y opens, which was the only way to reach it.
                        onLyricsRequested: root.openView("lyrics-current")
                ...
                Item {
                    ...
                    MouseArea {
                        id: crumbMouse
                        anchors.fill: parent
                        // NOT FROM UNDER SOMETHING. With a card, a prompt or a
                        // picture in front, the trail is a thing you are looking
                        // PAST rather than aiming at -- and hover is the half that
                        // got through whatever the stacking said, because a
                        // catcher accepts buttons and not pointer moves. So crumb
                        // steps lit up under a card, for a click that could never
                        // reach them; where no catcher spans the bar at all (a
                        // details sheet has only its own), the click reached them
                        // too and walked the trail underneath the sheet.
                        //
                        // `anythingUp` is the predicate that already answers "is
                        // there something in front of the rows", which is the same
                        // question here. The hover is cleared on the way down, or
                        // the last step the pointer touched would stay lit for as
                        // long as the overlay is up.
                        enabled: !root.anythingUp
                        onEnabledChanged: if (!enabled) root.crumbHover = -1
                        hoverEnabled: true
                ...
                Item {
                    ...
                    Image {
                        ...
                        onSourceChanged: {
                            coverFade.stop()
                            coverTop.opacity = 0
                            // NOTHING UNDERNEATH WHEN THE BACKDROP IS COMING BACK.
                            // coverUnder is the crossfade's floor -- the previous
                            // cover, so moving between two lists that each wear one
                            // dissolves from one to the other. But a grid wears no
                            // backdrop, and its floor stays whatever the last LIST
                            // wore: step from an album into the artist's grid and
                            // open another album, and that first album sat under
                            // the new one for the whole fade. The container is
                            // still at (or near) zero when that happens, so this is
                            // the test: hidden means there is nothing to dissolve
                            // FROM, and the new cover fades in over nothing.
                            if (contextCover.opacity < 0.5) coverUnder.source = ""
        ...
        // ═══ THE MODAL BARRIER ═══════════════════════════════════════════════
        //
        // ONE PLACE THAT ANSWERS "IS SOMETHING IN FRONT OF THE MENU", and the
        // reason this had to be written at all.
        //
        // Every widget on this panel was deciding that for itself. The rows had
        // `inert`, the crumb had `enabled: !anythingUp`, ctxLayer had a catcher of
        // its own, the card's list had a third rule -- and each named a DIFFERENT
        // subset of the four overlay states, because each was written the day a
        // particular leak was noticed. Everything nobody had noticed yet named
        // none at all: the wheel over the message bar still seeked the track
        // through a details sheet, a double click on the backdrop still opened the
        // art viewer through a card, the now-playing transport was still live
        // under both, and the trail still lit up under the pointer where no click
        // could ever reach it. Four leaks, four widgets, one cause.
        //
        // The cause is the shape: a per-widget opt-out is a list you have to
        // remember to add to, and a panel gains widgets. So the barrier is a LAYER
        // instead -- declared after everything the menu draws and before the things
        // that float over it, spanning the whole panel, eating every button, every
        // wheel notch and every pointer move the moment anything is up. A widget
        // added below this line is covered without knowing it exists, which is the
        // only version of this that stays fixed.
        //
        // A CLICK HERE IS A DISMISSAL, the same one `outside` and ctxLayer's own
        // catcher perform and through the same function -- with a sheet up and no
        // card, this IS what you click to put it away.
        //
        // `hoverEnabled` is not decoration: a MouseArea that accepts buttons still
        // lets pointer MOVES through to whatever is under it, which is the whole of
        // the trail lighting up beneath a card. Disabled it is transparent again,
        // so nothing is intercepted while the menu is the menu.
        //
        // The per-widget rules below it stay, as belt-and-braces rather than as the
        // mechanism -- the same standing this file already gives RowList.inert.
        ClickShield {
            id: modalGuard
            enabled: root.anythingUp
            // Hover as well as buttons: a MouseArea taking only buttons lets pointer
            // moves through, which is the trail lighting up beneath a card.
            hoverEnabled: true
            onDismissed: root.dismissTop()
        }
        ...
        Item {
            ...
            // function. `inert` stays, as belt-and-braces rather than the mechanism.
                // AS LONG AS THE CARD CAN BE SEEN, which is not the same as as
                // long as it is up.
                //
                // This was bound to `ctxUp || fieldUp` for a while, because a
                // catcher that outlived the card by the 140ms of its fade was
                // swallowing clicks after the card had logically gone while the
                // rows behind -- reading the state directly -- had already stopped
                // being inert, so the press reached neither. That is real, and
                // binding it to the state was the wrong half of the fix: it left
                // the opposite hole, where a click during the fade went STRAIGHT
                // THROUGH a card still on screen and picked the row under it.
                //
                // modalGuard is what closes the first hole now -- it and the rows
                // read one property, so they cannot disagree. That frees this to
                // do the honest thing and follow what is DRAWN: while any of the
                // card is still visible it takes the click, and dismissTop answers
                // false once there is nothing left to dismiss, so the press lands
                // on a card that is on its way out and does nothing at all. Which
                // is what a click on a dying card should do.
                //
                // Governed by the layer's own visibility rather than a copy of it:
                // an invisible item's children are not delivered events anyway, so
                // this is one fact, not two.
            ClickShield { onDismissed: root.dismissTop() }
                ...
                Rectangle {
                    ...
                    radius: zenon.radius
                    // Opaque, for the reason ctxCard is: this floats free of
                    // the panel and shares nothing behind it.
                ...
                Rectangle {
                    ...
                    radius: zenon.radius
                    // OPAQUE, because this floats free. See Theme.cardGround --
                    // written for exactly this and wired to one of the five things
                    // it describes.
                    //
                    // `ground` is 80% black on purpose: the panel is a HUD and the
                    // desktop is meant to show through it, which reads because
                    // everything ON the panel shares that one ground. A card does
                    // not. It overhangs the panel (that is the whole point of
                    // ctxLayer not clipping), so its edges sit over the desktop --
                    // and where it stays on the panel it covers the message bar and
                    // the now-playing line, neither of which the veil touches: the
                    // blur and the scrim are on the LIST. So the trail read through
                    // the top of a tall card and the desktop read through its
                    // overhang.
                    //
                    // The veil stays as it is. Pushing the list back is a different
                    // job from the card being a surface, and with an opaque card it
                    // is the thing that still reads around its edges.
                    color: zenon.cardGround
                    border.width: zenon.borderWidth
                    border.color: zenon.borderCol
                    ...
                    RowList {
                        ...
                        onPicked: function (i, alt) { if (root.ctxUp) root.activate(i, alt) }
                        // Nothing to queue: a card's rows are verbs.
                        onRowQueued: function (i) { if (root.ctxUp) root.activate(i, false) }
                        // THE CARD IS NOT ALWAYS THE THING IN FRONT. It was written
                        // as though it were -- this was the one list in the file
                        // with no `inert` at all, and the guard above reads ctxUp,
                        // which is still true under a details sheet or a picture
                        // opened from one of these very rows. So a click on the
                        // part of the card the sheet did not cover picked a verb
                        // out from under it.
                        //
                        // Same rule as both body lists, one level in: with
                        // something in front, these rows are what you click to
                        // DISMISS it. ctxUp is deliberately absent from the test --
                        // that is the state that makes this list live, not one that
                        // stands in front of it.
                        inert: root.viewerUp || root.promptFor.length > 0
                        // ONLY WHEN THIS LIST IS THE ONE BEHIND SOMETHING.
                        //
                        // rowClicked is NOT "a click on an inert row" -- RowList
                        // emits it at the end of an ordinary left click too, after
                        // moving the cursor. On the body lists that is harmless by
                        // accident: nothing is in front of them while they are
                        // live, so dismissTop finds nothing to dismiss.
                        //
                        // On the CARD's list it is never harmless, because the card
                        // itself is what dismissTop closes. Ungated it shut the card
                        // on the first click of any row -- every card in the app
                        // became unusable with the mouse.
                        //
                        // The rule was always "a click on a list you cannot see is
                        // how you put away the thing covering it", and `inert` is
                        // exactly that condition. Stated, rather than left to a
                        // coincidence about which list this happens to be.
                        onRowClicked: if (ctxList.inert) root.dismissTop()
                    }
        ...
        Rectangle {
            ...
            // Opaque, for the reason ctxCard is -- and this one is the case
            // that made it obvious: a sheet is opened from a LIST as often as from
            // a card (F1 from anywhere, Track Details from a row), so there is
            // frequently nothing but the menu behind it, and its rows read straight
            // through the ground.
            color: zenon.cardGround
            border.width: zenon.borderWidth
            border.color: zenon.borderCol
            ...
            // NOTHING GETS PAST A LAYER THAT IS IN FRONT -- ctxLayer's catcher
            // says exactly this, and the sheet had no counterpart to it.
            //
            // A sheet is opened from an action menu as often as from a list:
            // Track Details is a VERB in one, and the card is still standing when
            // the sheet arrives (the view answers with an event and no menu). Both
            // are placed by root.cardX/cardY, so the sheet lands squarely on top
            // of the card -- and a click on what looks like sheet fell straight
            // through to the card's rows underneath. One click moved a cursor you
            // could not see; two ran whichever verb it landed on. See ctxList's
            // `inert`, which is the other half of this.
            //
            // Declared FIRST so it is at the bottom of the sheet: the title bar
            // and the Flickable below take their own clicks above it. What reaches
            // here is a click on the sheet ITSELF, which is not a dismissal -- the
            // sheet is what you are reading -- so it is accepted and dropped.
            // Clicking AWAY still closes it: that misses this rectangle entirely
            // and lands on ctxLayer's catcher, on an inert row, or on `outside`,
            // all three of which call dismissTop.
            ClickShield { dismisses: false }
    ...
    Item {
        ...
        Rectangle {
            ...
            // Opaque, for the reason ctxCard is: this floats over the output
            // with nothing of spoot's behind it at all.
            color: zenon.cardGround
            ...
            Rectangle {
                ...
                // OPAQUE, because this is a MASK. It covers all of listenEdge's
                // gradient but its rim -- and at 80% it covered four fifths of it,
                // so the sweep went on showing faintly across the whole face of
                // the pill instead of being the line round its edge.
                color: zenon.cardGround
            }
