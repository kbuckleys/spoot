...
Window {
    ...
    // ONE THAT MERELY FLOATS OVER IT DOES NOT. The art viewer and the listener
    // draw their own card on the surface and leave the menu alone -- but this
    // switched on `overlayTheme`, which those two set as well, so an artist
    // impression squeezed the menu behind it from 1000px down to art.rasi's 640
    // and sprang it back on close. That is the main window being compressed.
    // Gated on overlayUp, the same test bodyHeight uses, so the two cannot
    // disagree about which overlays reshape the panel.
    readonly property var g: root.overlayUp ? zenon.geom(root.overlayTheme)
                                            : root.menuG
    ...
    Item {
        ...
        Rectangle {
            ...
            x: (!root.artIsListen || root.anchorH === "center")
                    ? Math.round((parent.width - width) / 2)
             : root.anchorH === "left" ? zenon.listenLift
             : parent.width - width - zenon.listenLift
            y: (!root.artIsListen || root.anchorV === "middle")
                    ? Math.round((parent.height - height) / 2)
            ...
            Image {
                ...
                layer.enabled: !root.artIsListen
