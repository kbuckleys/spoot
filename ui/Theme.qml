...
QtObject {
    ...
    // messageBg over a bar that paints no ground of its own.
    ...
    //
    // ...AND A `progressWash` FOLLOWED IT OUT. The grey the played portion of the
    // top bar was painted in: a full-width fill whose only informative part was
    // its leading edge, which had to be feathered over 34px so it would not read
    // as a cut -- so the number was carried by the blurriest thing on the bar, and
    // the contrast under the trail and the track name changed as it swept past.
    //
    // It is a line again, and green again, and where a line belongs: along the
    // panel's own top border and round both corners, revealed left to right. See
    // main.qml's progressLine.
    ...
    // SAVED AND NOT SAVED ARE ONE MARK, FILLED OR HOLLOW. The unliked state was a
    // hollow HEART -- a different symbol for the same fact, so the now-playing
    // line and the dock said "heart" where every list says "check". Same family
    // now: the filled circle-check means saved, the hollow one means not, and the
    // difference you read is the fill rather than a change of subject.
    // HOW FAR DOWN AN UNLIT CONTROL IS TURNED. Shuffle off, repeat off, a track
    // that is not saved: the same green as the lit ones, dimmed -- see GlyphKey,
    // where "off is faded, not grey" is the rule these follow.
    //
    // At 0.3 that reading failed at the bottom end. The panel's ground is already
    // translucent over whatever is behind it, so a glyph at three tenths is
    // competing with the desktop rather than sitting on a dark field, and an unlit
    // control read as an absent one. High enough now to be legible as a control
    // you could press, low enough that the lit ones still lead.
    readonly property real glyphDim: 0.55

    readonly property string glyphLiked:   "\u{F058}"
    readonly property string glyphUnliked: "\u{F05D}"
    // THE OTHER TWO STATUS MARKS, so the now-playing strip can draw each as an
    // item of its own rather than reading them back out of the engine's finished
    // `icons` string. Verbatim from Util.status_icons, which is still what every
    // list row and the dock wear -- these are the same three glyphs, and the two
    // must not drift.
    readonly property string glyphExplicit:     "\u{f071}"
    readonly property string glyphLyricsSynced: "\u{F0188}"
    readonly property string glyphLyricsPlain:  "\u{F0189}"
    ...
    // A ROW'S ICON IS EITHER A FILE OR A GLYPH, and the engine says which by
    // prefix -- see Util.ART_GLYPHS, which replaced the ten PNGs that used to be
    // shipped for rows with no real artwork. A real cover is always an absolute
    // path, so the two can never be confused.
    readonly property string artGlyphPrefix: "glyph:"
    function isArtGlyph(v) {
        return !!v && v.indexOf(artGlyphPrefix) === 0
    }
    // The character alone, for whatever is going to draw it as text.
    function artGlyph(v) {
        return isArtGlyph(v) ? v.substring(artGlyphPrefix.length) : ""
    }
    // ...and a glyph is NOT a file. Answered here rather than at each Image, so a
    // sentinel can never reach one as a source it would spend a load failing on.
    // HOW FAR ONE WHEEL NOTCH SCROLLS, in viewports. Snapped to whole rows by
    // Scroll.js, which every list in spoot answers the wheel through.
    //
    // Above one, so a notch clears the screen and crossing a six-hundred-row list
    // is a few flicks rather than fifty. Fast spins compound on top of this --
    // Scroll.js adds each notch to where the last one was HEADED rather than to
    // where the list currently is -- so this is the floor of the speed, not the
    // ceiling. Turn it down to 0.8 for the old behaviour, where a notch left a
    // couple of rows overlapping to tell you where you had been.
    readonly property real wheelStep: 1.6
    // ...AND A GRID NEEDS MORE OF THEM, because a viewport is not a fixed amount
    // of list. A list shows fourteen rows and a tile grid shows three, so one and
    // a half viewports is twenty-two rows in one and five in the other -- the same
    // number, a quarter of the travel. Measured: cellHeight 190 against a 570px
    // body. Expressed in viewports for both because that is what keeps a notch
    // from overshooting a short list, and given its own value here because that
    // unit hides a four-fold difference in what a notch actually covers.
    readonly property real wheelStepGrid: 3.0
    // ...and how long that move takes. Short enough that a spin reads as one
    // continuous movement rather than a series of separate jumps.
    readonly property int wheelMs: 90

    function fileUrl(path) {
        if (!path || !path.length || isArtGlyph(path)) return ""
        return "file://" + path
    }
