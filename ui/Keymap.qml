...
Item {
    ...
    Keys.onPressed: function (e) {
        ...
        if (alt && !ctrl) {
            ...
            // A CARD, not a place -- see app.openCard. The other two here are
            // real destinations: lyrics is a view you read, and art opens the
            // viewer over whatever you were on.
            case Qt.Key_E:        app.openCard("seek-current");       break
            // SEARCH, FROM WHEREVER YOU ARE. It was reachable one way -- the
            // Search tile on Main -- so looking something up from four levels
            // down meant walking home first, and the tile appends a root, so the
            // trail then carried the journey you took to get to a text field.
            //
            // As a CARD rather than a view, for the reason every other card here
            // is one: a box you type into is not a place you have gone. It floats
            // over whatever you were reading, the list underneath never moves,
            // and closing it costs nothing. The engine already serves this view
            // (SERVE_VIEWS.search); what changes is that nothing has to navigate
            // to reach it.
            case Qt.Key_F:        app.openCard("search");             break
        ...
        case Qt.Key_Backspace:
            ...
            //
            // ...AND AN EMPTY FIELD IS A FIELD YOU ARE LEAVING. With nothing
            // typed there is no character to delete, and the key fell through to
            // goBack -- which walks the TRAIL, out from under a prompt that was
            // still up. Backing out of "New Playlist" is what the press means
            // there, which is what Escape already does, so it does the same.
            if (app.composing && app.liveFilter.length) app.setFilter(app.liveFilter.slice(0, -1))
            else if (app.promptFor.length) app.cancelPrompt()
            else if (app.liveFilter.length) app.setFilter("")
            else app.goBack()
            break
        case Qt.Key_Return:
        ...
        // THE KEYMAP ITSELF, from wherever you are. It was a row in System, five
        // keystrokes from Main and further from anywhere else -- which is a
        // reference you have to already know your way around to reach. F1 is what
        // every other program puts it on.
        //
        // Not in the Alt block above: this key carries no text, so the typing
        // branch cannot swallow it and it needs no modifier to stay out of the
        // way. It answers with an event and no menu, so nothing is navigated to
        // -- see SERVE_VIEWS.keybinds.
        case Qt.Key_F1:      app.openView("keybinds"); break
