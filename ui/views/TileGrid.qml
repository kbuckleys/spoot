...
GridView {
    ...
    WheelHandler {
        ...
        onWheel: function (e) { Scroll.wheel(grid, e, wheelAnim, grid.theme.wheelStepGrid) }
    }
    NumberAnimation {
        id: wheelAnim
    ...
    delegate: Item {
        ...
        Column {
            ...
            Item {
                ...
                // A ROW THAT WILL NEVER HAVE A COVER WEARS A GLYPH. The engine
                // sends one as "glyph:<char>" rather than a path (see
                // Util.ART_GLYPHS), which is what the ten shipped PNGs became:
                // Search, System, Playback, Collections, Podcasts and the three
                // placeholders were pictures OF nerd icons, loaded off disk and
                // scaled into a 150px tile to draw something the font already
                // has. Drawn as text it is sharp at any tile size, takes the
                // theme's colour, and costs no image load at all.
                //
                // Sized off the plate rather than the theme's point size: this
                // stands in for a cover, so it fills the space a cover would.
                readonly property bool isGlyph: grid.theme.isArtGlyph(model.icon)

                Text {
                    anchors.fill: parent
                    visible: parent.isGlyph
                    text: grid.theme.artGlyph(model.icon)
                    horizontalAlignment: Text.AlignHCenter
                    verticalAlignment: Text.AlignVCenter
                    // ZENON's white, at full strength. It was the same colour at
                    // 0.55, which is not a white -- it is a grey, and a grey mark
                    // on a dark tile reads as a cover that failed to load rather
                    // than as the row's own icon.
                    color: grid.theme.foreground
                    font.family: grid.theme.fontFamily
                    // A MARK, NOT A FILL. At 0.42 of the plate this was a 63pt
                    // glyph in a 150px tile -- the icon wearing the tile rather
                    // than sitting in it, and next to a real cover, which fills its
                    // square edge to edge, the two did not read as the same kind of
                    // thing at all. A quarter of the plate leaves the air around it
                    // that says "this row has no artwork" without shouting it.
                    font.pointSize: Math.max(10, Math.round(grid.iconSize * 0.26))
                }
                ...
                // art settles into it.
                //
                // NOT UNDER A GLYPH, which is not a cover on its way: there is
                // nothing arriving for it, so the waiting plate would simply be
                // the tile's permanent background.
                Rectangle {
                    anchors.fill: parent
                    ...
                    visible: cover.status !== Image.Ready && !parent.isGlyph
                }
                // The cover is a TEXTURE, never drawn as itself: the shader
                // below samples it and paints it with its corners turned.
                Image {
                    id: cover
                    anchors.fill: parent
                ...
                // THE CORNER EVERY COVER TURNS. An Image has no radius and
                // `clip` is a bounding-box clip, so this was a MultiEffect mask --
                // which put every tile on its own offscreen layer, sixty render
                // targets for a screenful of covers, and one for every tile still
                // waiting for its art. A distance test in one shader pass does
                // the same corner with no layer at all. See shaders/roundart.frag.
                ShaderEffect {
                    anchors.fill: parent
                    property variant source: cover
                    property size size: Qt.size(width, height)
                    // PreserveAspectCrop is lost once the image is a texture --
                    // the texture is the whole loaded picture -- so the shader
                    // takes the centre square itself.
                    readonly property real aspect: cover.implicitHeight > 0
                        ? cover.implicitWidth / cover.implicitHeight : 1
                    property size crop: aspect > 1 ? Qt.size(1 / aspect, 1) : Qt.size(1, aspect)
                    property real radius: grid.theme.tileArtRadius
                    fragmentShader: Qt.resolvedUrl("../shaders/roundart.frag.qsb")
                    // Fades rather than pops. Late art should look intentional,
                    // which is the whole difference between streaming and
                    // stalling.
                    opacity: cover.status === Image.Ready ? 1 : 0
