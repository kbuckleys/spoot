...
GridView {
    ...
    WheelHandler {
        ...
        onWheel: function (e) { Scroll.wheel(list, e, wheelAnim, list.theme.wheelStep) }
    }
    NumberAnimation {
        id: wheelAnim
        duration: list.theme.wheelMs; easing.type: Easing.OutCubic
    }
    ...
    delegate: Rectangle {
        ...
        Text {
            ...
            anchors {
                ...
                right: marksCol.visible ? marksCol.left
                       : (metaCol.visible ? metaCol.left : copyMark.left)
                verticalCenter: parent.verticalCenter
                leftMargin: cell.playing ? 0 : list.theme.rowPadH
                // THE GUTTER IS THE ROW'S OWN INSET, not a number of its own: a
                // title that elides runs right up to the duration beside it, and
                // six pixels read as a collision rather than a column. The same
                // padding that holds the row off the left edge holds the label
                // off the column, so the two gaps match by construction.
                rightMargin: list.theme.rowPadH
            }
        ...
        // SAVED AND EXPLICIT, just inside the clock. Its own cell so that the
        // durations stay in their column whatever this row happens to carry, and
        // right-anchored so the marks themselves stay put too.
        Text {
            id: marksCol
            anchors { right: metaCol.visible ? metaCol.left : copyMark.left
                      rightMargin: metaCol.visible ? 0
                                 : (copyMark.showing ? list.theme.rowPadH : 0)
                      verticalCenter: parent.verticalCenter }
            visible: !list.pictorial && text.length > 0
            text: model.marks
            color: cell.playing ? list.theme.playing : list.theme.foreground
            Behavior on color { ColorAnimation { duration: 180 } }
            font.family: list.theme.fontFamily
            font.pointSize: list.rowSize
            font.weight: list.rowWeight
        ...
        Text {
            id: metaCol
            // HOW LONG, AND WHAT IS TRUE ABOUT IT -- the track's duration with its
            // saved and explicit marks in front of it, in a column of its own.
            //
            // The marks used to lead the title, and that is three constraints that
            // cannot all hold: in front they push every title to a different x (a
            // nerd glyph is 14.77px against a 9.59px space, so no padding closes
            // it), behind they elide away with a long title, and a reserved slot in
            // front leaves a hole on every row that has neither.
            //
            // The DURATION breaks it: every track already carries duration_ms, so
            // this column is OCCUPIED on every row rather than reserved, and the
            // marks ride in beside it. The label is anchored to its left edge, so a
            // long title elides against the column instead of over it.
            //
            // The CLOCK ALONE lives here, and the marks in their own cell to the
            // left, because a row carrying marks would otherwise push its duration
            // in by their width -- which is a duration column that does not line
            // up, the very thing this column was added to fix.
            anchors { right: copyMark.left
                      rightMargin: copyMark.showing ? list.theme.rowPadH : 0
                      verticalCenter: parent.verticalCenter }
            visible: !list.pictorial && text.length > 0
            text: model.meta
            // The row's own colour, so the playing row's column goes green with it.
            color: cell.playing ? list.theme.playing : list.theme.foreground
            Behavior on color { ColorAnimation { duration: 180 } }
            font.family: list.theme.fontFamily
            font.pointSize: list.rowSize
