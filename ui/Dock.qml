...
Window {
    ...
    // OPEN OR ABOUT TO BE. The one thing main.qml needs from a closed dock: its
    // playback poll runs only while something is showing it, and starting at the
    // TOUCH rather than at `open` gives the first answer the 220ms delay to land
    // in -- so the pill never comes up wearing whatever the last poll said,
    // however long ago that was.
    readonly property bool engaged: dock.open || openWait.running
