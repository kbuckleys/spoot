...
local function api_get(path, params, _retry)
    ...
    if status == 429 then
        ...
        if not (cool and os.time() < cool) and not Util.detached then
            ui_say("Spotify API rate limit reached (429). Retry after " .. secs .. "s.")
        end
        return nil
    end
    if status == 401 then
...
view_browse = function(entries, items, mesg, ctx, ctx_type, ctx_id, no_status, art_path)
    ...
    while true do
        ...
        local idx = ui_menu(entries, {prompt=ctx or "Browse", mesg=mesg, by_index=true, theme=view_theme, thumbs=is_album_grid or is_playlist_list or is_show_grid or is_artist_list or is_search_grid, items=items, entries=entries, ctx_type=ctx_type, ctx_id=ctx_id, refresh=rebuild, own_art=(art_path ~= nil) or nil,
