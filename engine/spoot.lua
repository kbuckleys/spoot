...
-- plant the pid files this reads and the files it writes through. The runtime
-- dir is the user's own (0700, on tmpfs, gone at logout), which is what these
-- files are: this session's, nobody else's. /tmp stays only as the fallback for
-- a system with no XDG runtime dir, namespaced by user so two accounts at least
-- do not collide.
P.run = os.getenv("XDG_RUNTIME_DIR") and (os.getenv("XDG_RUNTIME_DIR") .. "/spoot")
        or (P.tmp .. "/spoot-" .. (os.getenv("USER") or "user"))
...
-- THE ENGINE'S OWN MODULES, found beside it wherever it is installed: P.dir is
-- this file's directory, so require("lib.x") loads engine/lib/x.lua for the
-- engine and for every background job alike.
package.path = P.dir .. "?.lua;" .. package.path

-- Requests, the gates in front of them, notifications and the player: see
-- lib/transport.lua. Loaded here, before anything below can ask for any of it.
require("lib.transport")(Util, {P = P, json = json, SEP = SEP, read_file = read_file,
                                shell = shell, shell_quote = shell_quote, trim = trim})
...
-- Processes and markup -- pid files, the clipboard, escaping and stripping row
...
-- A TEMP NAME OF ITS OWN, per writer and per write. It was always `<file>.tmp`,
-- and there are several writers: the engine, up to three job states on their
-- own threads, a standalone helper outside the host. Two of them saving the same
-- file shared one temp -- the second open truncated what the first had written,
-- and one rename then moved the other's half-written bytes into place or found
-- nothing left to move. The rename is still what makes the write atomic; the
-- name only has to stop being shared. Four random bytes per Lua state, read
-- once, plus a counter.
--
-- `secret` makes the file 0600 BEFORE anything is in it. Chmod after the rename
-- left the refresh token world-readable on disk, in a 0755 directory, for the
-- length of a fork -- on every refresh -- and the temp file for its whole life.
-- An empty temp is created, locked down, then filled; rename keeps the mode.
local function write_file(p, d, secret)
    ensure_cache()
    if not Util._wtag then
        local u = io.open("/dev/urandom", "rb")
        local b = u and u:read(4); if u then u:close() end
        Util._wtag = b and #b == 4 and string.format("%02x%02x%02x%02x", b:byte(1, 4))
                     or tostring(os.time()) .. tostring({}):match("%x+$")
        Util._wseq = 0
    end
    Util._wseq = Util._wseq + 1
    local t = p .. ".tmp." .. Util._wtag .. "." .. Util._wseq
    if secret then
        local e = io.open(t, "w")
        if not e then return false end
        e:close()
        os.execute("chmod 600 " .. shell_quote(t) .. " 2>/dev/null")
    end
    local f = io.open(t, "w")
    if not f then os.remove(t); return false end
...
-- NOTHING EVER LEFT THIS TABLE UNLESS IT WAS ASKED FOR AGAIN.
--
-- An entry is dropped by mem_get when it is read AFTER expiring, or by mem_bust
-- by name. A key that is never read again is simply kept -- and the engine is
-- resident inside the GUI, so "never read again" is most of what goes in here.
--
-- What accumulates is not small. Util.status_icons asks Util.lyr_state about
-- EVERY ROW IT DRAWS, so one open of a 693-track Liked list mints 693
-- `lyrstate:` entries; a search stores the whole decoded result, six types of
-- fifty; every album, playlist and show you open keeps its track list. Browse for
-- an evening and the process is holding tens of megabytes of answers to questions
-- nobody is going to ask again.
--
-- A sweep rather than a cap, and amortised rather than on a timer: the cost of
-- walking this table is proportional to what is IN it, so paying it once every
-- 64 inserts is a fixed small share of the work that put things there. Expiry is
-- already recorded per entry -- this just stops waiting to be asked.
local _mem = {}
...
local function mem_set(key, value, ttl)
    -- The counter and the interval live on Util rather than as file locals: this
    -- chunk is at Lua's 200-local ceiling, which is the same reason
    -- Util.CATEGORY_BACKFILL_MAX and Util.pl_is_mine are where they are.
    Util._mem_writes = (Util._mem_writes or 0) + 1
    if Util._mem_writes >= 64 then
        Util._mem_writes = 0
        local now = os.time()
        for k, e in pairs(_mem) do
            if type(e) == "table" and e.expire and now >= e.expire then _mem[k] = nil end
        end
    end
...
-- What spoot remembers between launches -- settings, episode positions, cursors,
-- search and menu history: see lib/state.lua. After disk_get/disk_set, which it
-- is handed.
require("lib.state")(Util, {P = P, json = json, read_file = read_file,
                            write_file = write_file, safe_decode = safe_decode, trim = trim,
                            disk_get = disk_get, disk_set = disk_set})
...
local function cached_fetch(key, disk_path, ttl, fetch_fn, opts)
    ...
    if v == nil and (opts.stale_ok or Util.cache_only) and disk_path then
        v = disk_get(disk_path)
        if v ~= nil then
            -- A SHORT LEASE WHEN A FETCH ACTUALLY FAILED, taken for the reason
            -- the revalidate branch above takes one: re-arming the full TTL on a
            -- copy already judged out of date turns ONE refused request into half
            -- an hour of stale, and on a shared client id the gate is shut a good
            -- fraction of the time -- so that would be most of the session, and
            -- the copy would not refresh even once the window reopened.
            --
            -- Under cache_only nothing was attempted and nothing failed: that
...
toggle_repeat = function()
    ...
    Util.api_write_bg("PUT", Util.api_url("me/player/repeat", "state=" .. new_state),
...
toggle_shuffle = function()
    ...
    write_file(P.state, json.encode({repeat_state=repeat_state, shuffle=is_shuffle}))
    if not token then return end
    Util.api_write_bg("PUT", Util.api_url("me/player/shuffle",
...
-- The token as it stands ON DISK, refreshed if it has run out. Only ever called
-- through get_token below, which is what makes the disk read trustworthy: it
-- happens inside the lock, so it sees whatever the last refresher wrote.
Util.token_load = function()
    ...
    if data.expires_at and type(data.expires_at) == "number" then
        -- ...OR SPOTIFY HAS ALREADY SAID NO to this exact token (see api_get's
        -- 401), whatever its expiry claims: a clock that disagrees with
        -- Spotify's, or a token cut off early. Compared by value, so a token
        -- another state refreshed in the meantime is simply used.
        if os.time() > data.expires_at - 120
           or (Util.token_reject and data.access_token == Util.token_reject) then
            ...
            if data.refresh_token and type(data.refresh_token) == "string" then
                ...
                if rd and rd.access_token then
                    ...
                    -- CHECKED, because Spotify has just ROTATED the credential:
                    -- the refresh token on disk stopped working the moment this
                    -- reply arrived, and the only good copy is the one in `data`.
                    --
                    -- The answer was thrown away here. write_file returns false
                    -- for a full disk or a read-only home, and the session then
                    -- went on perfectly well from the copy in memory -- until the
                    -- NEXT launch refreshed with the dead token on disk, got
                    -- invalid_grant, and erased it. A silent sign-out, hours
                    -- later, from a failure nobody was told about. The paragraph
                    -- below is entirely about not losing this credential; this is
                    -- the other way it was being lost.
                    --
                    -- Tried twice, because the cheap causes are transient. If it
                    -- still will not land there is nothing to be done from here:
                    -- ui_say does not exist yet at this depth (see Util.rate_arm,
                    -- which says the same of itself), so it is recorded for the
                    -- System sheet to show and the session carries on working.
                    local wrote = Util.secure_write(P.token, json.encode(data))
                    if not wrote then wrote = Util.secure_write(P.token, json.encode(data)) end
                    Util.token_unsaved = not wrote
...
-- ONE REFRESHER AT A TIME, ACROSS EVERY LUA STATE IN THE PROCESS.
--
-- The engine and each background job are separate Lua states on separate
-- threads, and each keeps its own memo of the token -- so when it expired, every
-- one of them that happened to need it refreshed it independently, from the same
-- refresh token read off disk. Spotify may ROTATE that token on use. The second
-- refresh then answers invalid_grant, and the branch above that treats
-- invalid_grant as "revoked" deleted token.json -- which by then held the NEW
-- credential the first refresher had just written. A silent sign-out, found on
-- the next launch.
--
-- Serialised in the host (spoot.with_lock, a process-wide named mutex), with the
-- file read INSIDE the lock: whoever waited finds the refreshed token already on
-- disk, unexpired, and uses it instead of spending the old one. Outside a host
-- there is one process and nothing to race, so it runs bare.
get_token = function()
    local cached = mem_get("token")
    if cached then return cached end
    if Util.host and Util.host.with_lock then
        return Util.host.with_lock("token", Util.token_load)
    end
    return Util.token_load()
end
...
local function oauth_get_token()
    ...
    -- WAITING FOR THE ANSWER, one way or the other. Both hand back the callback's
    -- query string -- or nil after two minutes -- so everything below this is
    -- shared. The host has already turned away a wrong state; the check below is
    -- for the Perl path, which takes the first request it gets.
    local function await_code()
        if native then return Util.host.oauth_wait(120, state) end
        local attempts = 0
    ...
    local query = await_code()
    if not native then kill_oauth_server(); os.remove(P.oauth_code) end
    if not query or query == "" then
        ui_say("OAuth timed out — no response after 120 seconds")
        return nil
    end
    local function param(k) return ("&" .. query):match("&" .. k .. "=([^&]*)") end
    -- Cancel on Spotify's consent page comes back as error=access_denied, and
    -- used to leave this waiting out the full two minutes for a code that was
    -- never going to arrive.
    if param("error") then
        ui_say("Login cancelled")
        return nil
    end
    if param("state") ~= state then
        ui_say("Login answer did not match this login -- try again")
        return nil
    end
    ...
    Util.secure_write(P.token, json.encode({
...
-- Cover urls and cover files: see lib/art.lua.
require("lib.art")(Util)
...
-- Artwork fetched and filed -- batches, the prefetch spool, the per-kind index,
-- the glyph stand-ins and the grid decorator: see lib/artwork.lua.
require("lib.artwork")(Util, {P = P, read_file = read_file, write_file = write_file,
                              shell = shell, shell_quote = shell_quote,
                              ensure_cache = ensure_cache, disk_get = disk_get,
                              disk_set = disk_set})

-- SPOTIFYD MANAGEMENT

-- Six hours, where it was ten minutes. spotifyd's device keeps its id for as
-- long as it keeps its name, so re-resolving every ten minutes was a
-- me/player/devices request bought to learn nothing -- and a stale id is not
-- silent: Spotify answers 404, and do_play and do_add_queue both drop the id
-- and ask again on exactly that.
P.device_ttl = 6 * 3600
...
local function get_spotifyd_device()
    ...
    local d = safe_decode(Util.http{
        url = Util.api_url("me/player/devices"), compressed = true,
...
local function ensure_spotifyd()
    ...
    if pid == "" then
        os.execute("env 'PULSE_PROP_application.name=spoot'"
            ...
            -- A CEILING ON THE TRACK CACHE. Without one librespot keeps every
            -- track it ever streamed -- this machine's reached 15 GB once, and
            -- 1.4 GB again since the Track Cache row last emptied it. With one it
            -- evicts the least recently used files itself, at start and as it
            -- writes. 20 GiB, Buck's call: a few thousand tracks at 320 kbps,
            -- and still a ceiling.
...
local function api_get(path, params, _retry, _reauthed)
    ...
    if status == 401 then
        -- ONE REFRESH, THEN ONE RETRY. An access token can be refused before the
        -- time it said it would expire -- a clock that runs ahead or behind, a
        -- token Spotify cut short -- and the refresh token would fix that
        -- without asking anything of you. Only when a fresh token is refused
        -- too, or none can be had, is Re-authenticate the answer.
        if not _reauthed then
            Util.token_reject = token
            mem_bust("token")
            local fresh = get_token()
            if fresh and fresh ~= token then return api_get(path, params, _retry, true) end
        end
    ...
    if status >= 500 and not _retry then
        ...
        Util.wait(1)
        return api_get(path, params, true, _reauthed)
    end
...
Util.paged_fetch = function(path, mk_params, done, each, opts)
    ...
    -- opts.first is page one ALREADY IN HAND, for an endpoint that embeds it in
    -- a parent object -- shows/{id} carries its first page of episodes -- so the
    -- pager does not ask for the same fifty twice.
    local d = opts.first or api_get(path, mk_params(0))
    ...
    if per > 0 and total and token then
        ...
        for i = 1, pages - 1 do
            ...
            local dn = (r and Util.is2xx(r.code)) and safe_decode(r.body) or nil
...
local function parallel_fetch_library()
    ...
    -- EVERY PAGE IN MEMORY. They used to be files in tmpdir, written by the
    -- batch, read back, decoded and deleted -- the directory itself a rm and a
    -- mkdir fork per sync. `pages` holds each 2xx body under the name its file
    -- would have had, so the phases below read it exactly as they read the file.
    -- The directory is only needed when curl is the transport: curl can only
    -- write files, and curl_batch reads them back for us.
    local pages = {}
    local via_curl = not (Util.host and Util.host.http) or os.getenv("SPOOT_FORCE_CURL")
    if via_curl then
    ...
    local function fire_batch(jobs)
        ...
        local got = Util.curl_batch(jobs, {compressed = true, parallel = 4, timeout = 10,
                                           bodies = true, header = auth})
        if got then
            -- Kept before the 429 test, as the files were: a limited wave still
            -- delivered whatever pages came back whole.
            for _, j in ipairs(jobs) do
                local r = got[j.out]
                if r and Util.is2xx(r.code) and r.body then pages[j.out] = r.body end
            end
            for _, j in ipairs(jobs) do
                local r = got[j.out]
    ...
    local function page_valid(file)
        local d = safe_decode(pages[file])
        return d and d.items and #d.items > 0
    end

    -- PHASE 1: PROBE — fetch page 0 for all three endpoints simultaneously
    fire_batch({
        {url = Util.api_url("me/tracks", "limit=50&offset=0" .. mkt), out = tmpdir .. "/lk_0.json"},
        {url = Util.api_url("me/albums", "limit=50&offset=0" .. mkt), out = tmpdir .. "/al_0.json"},
        {url = Util.api_url("me/following", "type=artist&limit=50"), out = tmpdir .. "/ar_0.json"}
    })
    -- PHASE 2: PARSE PROBES — extract totals and page 0 data
    local lk0 = safe_decode(pages[tmpdir .. "/lk_0.json"])
    local al0 = safe_decode(pages[tmpdir .. "/al_0.json"])
    local ar0 = safe_decode(pages[tmpdir .. "/ar_0.json"])
    ...
    for i = 1, tracks_pages - 1 do
        jobs[#jobs+1] = {url = Util.api_url("me/tracks", "limit=50&offset=" .. (i * 50) .. mkt),
                         out = tmpdir .. "/lk_" .. i .. ".json"}
    end
    for i = 1, albums_pages - 1 do
        jobs[#jobs+1] = {url = Util.api_url("me/albums", "limit=50&offset=" .. (i * 50) .. mkt),
    ...
    local function retry_pages(pages, prefix, path)
        for i = 1, pages - 1 do
            ...
            if not page_valid(f) then
                ...
                Util.wait(1)
                if not fire_batch({{url = Util.api_url(path, "limit=50&offset=" .. (i * 50) .. mkt),
    ...
    while artist_after and artist_after ~= "" do
        ...
        local ar_page = safe_decode(Util.http{
            url = Util.api_url("me/following", "type=artist&limit=50&after=" .. artist_after),
    ...
    for i = 0, tracks_pages - 1 do
        local raw = pages[tmpdir .. "/lk_" .. i .. ".json"]
    ...
    for i = 0, albums_pages - 1 do
        local raw = pages[tmpdir .. "/al_" .. i .. ".json"]
        local d = raw and safe_decode(raw)
        if d and d.items and #d.items > 0 then
    ...
    if via_curl then os.execute("rm -rf " .. shell_quote(tmpdir)) end
    return
...
-- DOES THE CACHE ON DISK ALREADY EXPLAIN THIS PROBE?
--
-- A like or unlike made inside spoot patches liked_tracks in place and writes no
-- fingerprint (see do_like). So the next probe reads a library that has moved,
-- the revalidator below calls it changed, and all 693 tracks are re-paged --
-- fourteen requests six at a time, in a burst, for a change this app performed
-- itself and had already recorded correctly. That burst is the one thing here
-- big enough to earn a 429 on its own, and earning it shuts the global gate on
-- the foreground: the list stops updating and play refuses.
--
-- The probe already in hand answers it with no second request. If the total and
-- the head of the list match what is on disk, the cache IS the library and the
-- only thing missing was the fingerprint.
--
-- LIKED AND ALBUMS ONLY. Those two probe as "<total>:<id>:<added_at>", which is
-- comparable against a stored list; the artists probe is a positional hash of a
-- whole page (see Util.lib_probe) and reproducing it here would be a second copy
-- of that arithmetic waiting to drift from the first.
function Util.lib_fp_matches(kind, probe)
    if kind ~= "liked" and kind ~= "albums" then return false end
    if type(probe) ~= "string" then return false end
    local total, id, added = probe:match("^(%-?%d+):([^:]*):(.*)$")
    total = tonumber(total)
    if not total or id == "" then return false end
    local list = disk_get(kind == "liked" and P.liked or P.albums)
    if type(list) ~= "table" or #list ~= total then return false end
    local head = list[1]
    if type(head) ~= "table" or head.id ~= id then return false end
    -- Liked tracks keep their added_at, so that match can be exact. Saved albums
    -- do not store theirs, and total plus head is all there is to compare.
    if kind == "liked" and added ~= "" and head.added_at ~= added then return false end
    return true
end
...
Util.REVALIDATORS.library = function()
    ...
    -- A KIND WHOSE CACHE ALREADY MATCHES ITS PROBE HAS NOT MOVED, whatever the
    -- fingerprint says -- that is a like made here, not an edit made elsewhere.
    -- See Util.lib_fp_matches.
    local moved, drifted = false, false
    for _, kind in ipairs({"liked", "albums", "artists"}) do
        if now[kind] ~= fp[kind] then
            drifted = true
            if not Util.lib_fp_matches(kind, now[kind]) then moved = true end
        end
    end
    local changed = moved
    ...
    if not changed then
        ...
        if Util.cache_touch(P.liked) and Util.cache_touch(P.albums)
           and Util.cache_touch(P.artists) then
            -- ...and write down what we just agreed to, so the next probe has no
            -- difference left to explain and this cannot re-decide it every
            -- fifteen minutes for the rest of the day.
            --
            -- `full_at` is CARRIED OVER, not refreshed: nothing was re-paged, and
            -- moving it would push the twelve-hour full rebuild out by fifteen
            -- minutes every time a track was liked -- which is the one thing that
            -- catches whatever a probe cannot see.
            if drifted then
                now.full_at = fp.full_at
                disk_set(P.lib_fp, now)
            end
            return
        end
    end
...
--
-- WHOSE POOL THAT SPENDS: the APP's, and this is now measured rather than
-- argued. Two runs, same account, same endpoint (me/tracks?limit=1), same rate
-- of one request every twenty seconds, forty minutes apart:
--
--   * the bundled id -- 8 of 12 refused, the successes arriving one a minute
--     like a metronome, which is what a pool already drained by other installs
--     looks like from inside one of them.
--   * a freshly registered id, same account -- 0 of 12 refused. Then 60
--     requests 12 at a time: 0 refused, 1.7s, ~35/s.
--
-- The account is the thing held constant, so it is not the account. An earlier
-- note here concluded the opposite from a single uncontrolled reading ("one
-- request in six was still refused"); that reading is superseded by this pair.
--
-- What does NOT follow: that a private id is limitless. The burst above is a
-- short one against a development-mode app, and says nothing about sustained
-- load over hours.
--
-- Which leaves this function exactly where it was. Sustained polling is what
-- earns a limit, and the limit is rolling, so asking less when there is nothing
-- to hear is right whoever the quota belongs to -- and on the shared id, where
-- ~1 request a minute gets through, it is the difference between spoot working
-- and spoot not.
--
...
-- once there are any.
-- A TRACK'S LENGTH, as a clock. The details sheet spelled this out inline and it
-- is now also a list column, which is exactly the point at which a second copy
-- starts drifting -- so it lives here beside dur_short, which answers the other
-- question (how LONG is this, coarsely) for rows that want prose.
function Util.dur_clock(ms)
    ms = tonumber(ms) or 0
    if ms <= 0 then return "" end
    return string.format("%d:%02d", math.floor(ms / 60000),
...
function Util.display_episode(item, hide_artist, hide_liked, hide_single_artist)
    ...
    -- HOW LONG AND HOW FAR IN goes to the right-hand column, not into the line:
    -- the same column a track's duration lands in, so a podcast list and a track
    -- list read the same way down the right edge. Written into the line it would
    -- elide away on a long episode title -- which is exactly when it is wanted.
    ...
    -- The column rides as a SECOND return, packed the way display_track packs
    -- its own -- "<marks>\x1f<column>", with no marks on an episode. A caller
    -- that concatenates takes the first value and is unchanged.
    return txt, "\x1f" .. meta
end
...
display_track = function(item, hide_artist, hide_liked, hide_single_artist)
    ...
    --
    -- The duration is what breaks the deadlock. Every track object already carries
    -- duration_ms (693 of 693 in the liked cache), so a right column is never
    -- empty and never reserved: it is occupied by something useful on every row,
    -- and the marks ride in the gap beside it. Titles then all start at the same
    -- column, nothing is padded, and nothing elides -- RowList bounds the label at
    -- this column's left edge.
    --
    -- Marks first, then the clock, so the DURATION is the flush-right edge and the
    -- marks float inward from it. The other way round leaves a ragged margin.
    local l  = (not hide_liked) and item.id and liked[item.id] and "\u{F058} " or ""
    local e  = item.explicit and "\u{f071} " or ""
    -- Packed as "<marks>\x1f<duration>" so the UI can put the CLOCK flush right
    -- and let the marks float in beside it. One string rather than two channels;
    -- serve_rows splits it into the two fields RowList draws.
    local meta = l .. e .. "\x1f" .. Util.dur_clock(item.duration_ms)
    local txt = (item.name or "Unknown") .. (hide and "" or SEP .. an)
    ...
    -- The column rides as a SECOND return. A caller that concatenates takes the
    -- first value and is unchanged; the row builders hand it to Util.pack_meta.
    return txt, meta
end
...
function Util.format_mixed_item(t, i)
    ...
    local body, meta
    if st == "tracks" then body, meta = display_track(t)
    ...
    -- With its column, like the tracks arm: an episode's length moved out of the
    -- line when the column arrived, so taking only `body` here dropped it.
    elseif st == "episodes" then body, meta = Util.display_episode(t)
    else body = display_playlist(t) end
    return Util.pack_meta(string.format("%2d. %s", i, pfx .. body), meta)
end
...
format_entries = function(tracks, hide_artist, hide_liked, hide_single_artist)
    ...
    for i, t in ipairs(tracks) do
        local body, meta = display_track(t, hide_artist, hide_liked, hide_single_artist)
        entries[i] = Util.pack_meta(string.format("%2d. %s", i, body), meta)
    end
...
-- STATUS, POSITION AND VOLUME IN ONE D-BUS CALL. The playback poll reads all
-- three every second, and each one was its own round trip; a GetAll answers them
-- together and fills all three one-second memos below at once. Any reader whose
-- memo was busted (a seek, a pause, a volume change) simply asks again, and the
-- next GetAll refills the lot. A field the player leaves out of GetAll is left
-- unset, and its reader falls back to its own single call.
--
-- A player that is not there answers the same "nothing" all three would have, so
-- that is memoised too rather than paid for three more times. The playerctl
-- branch has no GetAll and says so with `unsupported`; nothing is memoised then.
function Util.player_state()
    local r = Util.mpris{op = "state"}
    ...
    elseif not r.ok then mem_set("_playerctl_vol", get_saved_volume(), 1) end
    return v
end
...
function Util.playerctl_status()
    ...
    if cached ~= nil then return cached end
    Util.player_state()
    cached = mem_get("_playerctl_status")
    if cached ~= nil then return cached end
...
function Util.status_icons(item)
    if not item then return "" end
    local out = ""
    if Util.is_liked(item.id) then out = out .. " \u{F058}" end
...
local function do_play(item, ctx_type, ctx_id, all_items, idx)
    ...
    -- A PARAMS STRING, not a URL fragment. Util.api_url owns the "?" and is
    -- "the one place a v1 endpoint becomes a URL" by its own note -- writes
    -- were spelling the host out themselves, which is the second copy that
    -- note warns about.
    local dparam = device_id and ("device_id=" .. device_id) or nil
    ...
    if body then
        -- NOT INTO A WINDOW WE ALREADY KNOW IS SHUT.
        --
        -- Writes are ungated by design and the note on the gate says why: a like
        -- or a queue is rare, deliberate, and worth one refusal rather than being
        -- silently dropped. A PLAY is neither rare nor harmless. It is the button
        -- you press again when nothing happened, and every press inside a closed
        -- window is what makes Spotify lengthen it. Refusing to send is what lets
        -- the window expire instead of being renewed.
        ...
        if code and code:match("429") then Util.rate_arm(nil, phdr) end
        ...
        -- A 429 HERE IS THE ACCOUNT SAYING "LATER", and it was the one 429 in the
        -- file that armed nothing. Writes are deliberately not GATED -- a play is
        -- something you just asked for -- but the answer still tells us the pool
        -- is spent, and every READ behind it walking into the same wall is what
        -- turns one throttle into a standing one. Arming also makes Util.rate_why
        -- able to name the real reason instead of a bland failure.
...
-- WHY A TRANSPORT CALL DID NOTHING, AND IT IS NEVER THE RATE LIMIT.
--
-- The four sites that report Util.transport's failure did it through
-- Util.rate_why, which names the Spotify gate whenever the gate happens to be
-- shut. Util.transport is the D-Bus call directly above: one synchronous message
-- to the local player, no network anywhere near it, and its `false` means the
-- player did not answer -- spotifyd down, restarted, or without a bus name.
--
-- On a shared client id the gate is shut a good fraction of the time, so this
-- reported a dead player as a rate limit most of the time it was wrong, which
-- sends you to look at your quota instead of at the thing that is actually
-- broken. Named once, because four copies of a sentence drift.
function Util.transport_why(verb)
    return "Failed to " .. verb .. SEP .. "no answer from the player"
end
...
-- spoot happens to believe about it.
--
-- One D-Bus read on this machine: no network, no quota, and true after a cold
-- start where `current_id` names what SPOTIFY last remembers and `played_here` is
-- still false because nothing has played through spoot yet.
function Util.player_has(id)
    if not id or not Util.player_live() then return false end
    local snap = Util.mpris{op = "metadata"}.value
    local raw = snap and snap.trackid
    if not raw then return false end
    return (Util.extract_track_id(raw)) == id
end
...
function Util.play_or_toggle(item, ctx_type, ctx_id, all_items, idx)
    ...
    -- ...AND THE ANSWER IS READ, which is the thing the two notes above were
    -- reaching for and could not get at from a precondition.
    --
    -- Both of them narrow "is a toggle the right move here" by asking about state
    -- BEFORE sending: is a player on the bus, has anything played here. Each fixed
    -- a real case and neither can be complete, because the question is not what is
    -- true a moment before -- it is whether the player takes the message. A daemon
    -- that restarted, dropped its bus name or wedged passes every check and then
    -- does nothing, and this branch answered `true` for it: the row reported a
    -- success that never happened, nothing played, and no message said so.
    --
    -- That is "sometimes I cannot play anything unless I Shift+Return the track
    -- and pick Play". Play in that menu is do_play -- it STARTS the track over the
    -- Web API rather than asking the local player to resume -- so the menu worked
    -- on precisely the occasions the row did not.
    --
    -- Util.transport already reports what happened (it reads the state back off
    -- MPRIS rather than assuming). So a toggle that did not land falls through to
    -- exactly what the menu does, and the row and the menu stop disagreeing.
    -- ASK THE PLAYER, NOT OUR OWN BOOKKEEPING.
    --
    -- This tested `item.id == current_id` and `is_playing or Util.played_here` --
    -- three beliefs about what is loaded, none of which is the player. After a
    -- cold start every one of them can be wrong in the same direction: spotifyd is
    -- running with your last track loaded and paused, `played_here` is false
    -- because nothing has played through spoot yet, so this branch was skipped and
    -- Return went to the Web API to start a track the machine already had.
    --
    -- That is the whole of "Return says it cannot play but the card's Resume
    -- works": Resume is Util.transport, which is MPRIS and local, and the row was
    -- spending a request to do the same thing -- one that comes back 429 on a
    -- throttled app and then cannot be done at all.
    --
    -- Util.player_has is one D-Bus read and costs no quota, so the commonest press
    -- in the app -- resume what is already loaded -- now touches the network
    -- nowhere.
    if item.id and Util.player_has(item.id) then
        local playing = (Util.mpris{op = "status"}.value or "") == "Playing"
        if Util.transport(not playing) then return true end
    end
    -- Read BEFORE the play: a gate already shut is one do_play refuses at the
    -- door, and it says so itself, with the seconds left. A gate the play shuts
    -- (its own 429) it does not speak about, so that case still falls through.
    local gated = Util.rate_cool() > 0
    if do_play(item, ctx_type, ctx_id, all_items, idx) then
        Util.adopt_playing(item)
        return true
    end
    -- ...and NOT A SECOND TIME. This used to follow the refusal with "Could not
    -- play that · rate limited", which replaced the one message that said how
    -- long to wait.
    if gated then return false end
    -- AND SAY SO, because nobody else will. do_play answers false without a word
    -- -- no token, an unavailable track, a device that refused the request -- and
    -- every one of the five callers here throws this answer away, so a Return that
    -- could not play was completely silent. Said HERE rather than at the five call
    -- sites, for the reason the file gives everywhere else it does this: a route
    -- that forgets to report cannot then exist.
    --
    -- Wrapped in Util.rate_why so the commonest real cause names itself rather
    -- than hiding behind a generic sentence -- the same treatment view_playback
    -- gives Failed to skip and Failed to pause.
    ui_say(Util.rate_why("Could not play that"))
    return false
end
...
-- Everything a clean exit does except the exit itself -- split out for the
-- hosted Quit, which must not end the process from the engine's thread.
...
local function do_like(item, unlike)
    ...
    local url = Util.api_url("me/tracks", "ids=" .. item.id)
...
local function do_follow_artist(artist_id, follow, artist)
    ...
    local url = Util.api_url("me/following", "type=artist&ids=" .. artist_id)
...
local function do_add_queue(item)
    ...
    local device_id = get_spotifyd_device()
    local function post(dev)
        local url = Util.api_url("me/player/queue", "uri=" .. uri)
            .. (dev and ("&device_id=" .. dev) or "")
        return Util.api_write("POST", url, token, {len0=true})
    end
    local r = post(device_id)
    -- 404 is "no such device": the remembered id went stale. Same recovery
    -- do_play has -- forget it, resolve it again, and try once more.
    if r and r:match("404") and device_id then
        Util.bust_device()
        local fresh = get_spotifyd_device()
        if fresh and fresh ~= device_id then r = post(fresh) end
    end
...
function Util.lib_write(kind, id, save, item)
    ...
    local url = Util.api_url(k.ids, "ids=" .. id)
...
recover_playback = function(direction, force)
    ...
    local body
    -- A PLAYLIST'S CONTEXT IS NOT TRUSTED HERE EITHER. do_play stopped playing
    -- playlists through their context_uri, because the live playlist and the
    -- cached one spoot drew routinely disagree -- and an offset into a playlist
    -- that has since changed plays whatever now sits there, or its first track.
    -- This resume path still did exactly that, by position. Playlists resume from
    -- our own list instead (the uris branch), which is what do_play started them
    -- from in the first place.
    --
    -- An album context is still used, and entered BY URI rather than position:
    -- queue_idx indexes our filtered list, and one id-less row ahead of the
    -- track put a position one off.
    local ctx_ok = queue_context and not queue_context:find("^spotify:playlist:")
...
function Util.api_get_show(show_id)
    return cached_fetch("show_" .. show_id, P.mass .. "/show_" .. show_id .. ".json", CACHE_TTL_LONG, function()
        ...
        if not d then return nil end
        -- THE BACK CATALOGUE, IN PARALLEL. This followed `next` one page at a
        -- time, so a show with two thousand episodes was forty back-to-back
        -- requests before its list could draw -- twenty seconds at ~500ms each,
        -- with the engine answering nothing else. Util.paged_fetch knows the
        -- page count from `total` and fans the rest out six at a time, and it
        -- already owns what this loop did not: the rate gate before the burst,
        -- a 429 arming it, and a retry for any page the batch lost. Page one
        -- came with the show, so it is handed over rather than asked for again.
        local eps = {}
        if d.episodes and d.episodes.items then
            -- The embedded page's OWN size: the pager steps by it, so asking for
            -- a different limit would overlap pages and repeat episodes.
            local lim = tonumber(d.episodes.limit) or 50
            eps = Util.paged_fetch("shows/" .. show_id .. "/episodes",
                function(o) return Util.with_market("limit=" .. lim .. "&offset=" .. o) end,
                function(pg, items) return #items == 0 or not pg.next end,
                nil, {first = d.episodes})
...
Util.view_track_details = function(item)
    ...
    if d.duration_ms then
        s.add("Duration", Util.dur_clock(d.duration_ms))
    end
...
api_get_playlist_tracks = function(playlist_id, snapshot)
    return cached_fetch("playlist_tracks_" .. playlist_id, P.mass .. "/playlist_tracks_" .. playlist_id .. ".json", 1800, function()
        return Util.paged_fetch("playlists/" .. playlist_id .. "/tracks",
            ...
            --
            -- `total` too, and it is the whole of "playlists load one page at a
            -- time". Util.paged_fetch fans the remaining pages out in parallel only
            -- when page one tells it how many there are; this mask dropped the
            -- field, so every playlist fell through to the one-page-at-a-time
            -- cursor walk that function exists to replace.
            function(o) return Util.with_market("limit=100&offset=" .. o .. "&fields=total,items(track(id,name,type,duration_ms,explicit,is_playable,artists,album(id,name,images,artists)),added_at),next") end,
            function(d, items) return #items == 0 or not d.next end,
            function(entry)
    ...
    -- STALE BEATS "Failed to load playlist". The tag is the snapshot id, so an
    -- unchanged playlist is served fresh however old the file is and this branch
    -- is only reached when the playlist really did move and the refetch failed.
    -- The tracks from before the edit are a far better answer than a red message
    -- over an empty list, and the next open that gets through corrects them.
    end, {tag = snapshot, stale_ok = true})
end
...
-- What a search is -- its types, pages, query and cache names: see lib/search.lua.
require("lib.search")(Util, {P = P, mem_bust = mem_bust, shell_quote = shell_quote,
                             url_encode = url_encode})

local function api_search(query)
    local params, key = Util.search_query(query)
    -- The same query searched again while the gate is shut answered "Search
    -- failed" on top of a cached result set sitting right there. Results for a
    -- query you have already run do not go bad in a way that makes yesterday's
    -- copy worse than nothing.
...
function Util.search_prefetch(queries)
    ...
    ensure_cache()
    -- THE GATE, IMMEDIATELY BEFORE THE BURST, exactly as Util.paged_fetch and
    -- parallel_fetch_library's fire_batch take it. This was the one
    -- Util.curl_batch caller in the file that took neither it nor the 429 on the
    -- way back -- and it is not a small one: Util.run_shelf_warm hands it all
    -- twenty-one PODCAST_TOPICS at once, against /search, in a job that launches
    -- beside the library sweep. A wave that came back limited armed nothing and
    -- told nobody, so every other reader of the gate went on walking into the
    -- same closed window; see the note above Util.rate_arm, which is that report.
    if Util.rate_cool() > 0 then return end
    ...
    -- Four in flight rather than eight, and for parallel_fetch_library's reason
    -- rather than a new one: every job here hits the SAME endpoint, which is the
    -- shape a per-endpoint limiter punishes hardest, and nobody is watching a
    -- spinner for a prefetch. Util.paged_fetch keeps six because a person is.
    local got = Util.curl_batch(jobs, {compressed = true, parallel = 4, timeout = 15,
                                       bodies = true, header = auth})
    -- A LIMITED WAVE ARMS THE GATE AND GIVES UP ON THIS SWEEP. Asking again
    -- inside a rolling window is what extends it, and a missing prefetch costs
    -- nothing anyone can see: api_search fetches the query when the topic is
    -- actually opened, which is what it did before this function existed.
    if got then
        for _, w in ipairs(want) do
            local r = got[w.tmp]
            if r and r.code == "429" then
                for _, x in ipairs(want) do os.remove(x.tmp) end
                -- Silent, like fire_batch's: this runs on a background warm, and
                -- Util.rate_arm says the notice belongs to whoever is looking at
                -- a screen. The GATE is the point.
                Util.rate_arm()
                return
            end
        end
    ...
    for _, w in ipairs(want) do
        local r = got and got[w.tmp]
        local d = (r and Util.is2xx(r.code)) and safe_decode(r.body or "") or nil
...
-- ONE "MAKE A PLAYLIST", because there were two and they had drifted.
--
-- Both posted the same body and then read the answer differently: Add to
-- Playlist required an `id` on what came back, the Playlists view accepted
-- anything that DECODED. Spotify reports a refusal as a JSON object -- an
-- {"error": {...}} body -- which decodes perfectly well, so the second one took
-- a failure for a new playlist: it appended the error object to the list as a
-- row, handed it to bust_my_playlists (which splices it into the cached shelf
-- and writes that to disk), and said nothing at all. The result was a nameless,
-- idless playlist on your shelf until the cache expired.
--
-- `id` is the test, not decodability. Answers the created playlist or nil, and
-- nil is the only failure shape there is.
function Util.create_playlist(name, token, owner_id)
    if not (name and #name > 0 and token) then return nil end
    if not owner_id then
        local me = api_get_me()
        owner_id = me and me.id
    end
    if not owner_id then return nil end
    local r = Util.api_write("POST", Util.api_url("users/" .. owner_id .. "/playlists"),
                             token, {body = {name = name}, raw = true})
    local cr = safe_decode(r)
    if type(cr) ~= "table" or not cr.id then return nil end
    return cr
end
...
function Util.build_pl_index()
    ...
    for _, p in ipairs(pls) do
        if p.id and Util.pl_is_mine(p, my_id) then
            ...
            -- WITH THE SNAPSHOT, which me/playlists hands over for free. Without it
            -- the cache fell back to its 1800s TTL, so every rebuild (up to one per
            -- five minutes) re-paged each of your playlists older than half an
            -- hour -- and rewrote its file with no tag, which knocked the playlist
            -- view's own snapshot check back to the TTL as well. With it an
            -- unchanged playlist is a disk read however old, and only one that
            -- really moved goes to the network.
            for _, t in ipairs(api_get_playlist_tracks(p.id, p.snapshot_id) or {}) do
                if t.id then
...
-- Read-only and non-blocking: view_actions must never wait on a walk of every
-- playlist before it draws.
...
function Util.do_remove_from_playlist(playlist_id, track_id)
    ...
    local url = Util.api_url("playlists/" .. playlist_id .. "/tracks")
...
local function api_get_category_playlists(cat_id)
    return cached_fetch("category_playlists_" .. cat_id, P.mass .. "/category_playlists_" .. cat_id .. ".json", CACHE_TTL_MED, function()
        ...
        -- ALL AT ONCE, NOT ONE AFTER ANOTHER. There is no batch ENDPOINT for
        -- playlists -- the note above CATEGORY_BACKFILL_MAX is right about that --
        -- but Util.curl_batch is not an endpoint, it is twenty requests over one
        -- connection with a concurrency bound. Sequentially this was twenty round
        -- trips at ~200ms, so opening Made For You cold blocked the engine for
        -- four seconds; the engine is single-threaded, so the playback poll and
        -- every other command queued behind it for all four.
        --
        -- Same shape as Util.paged_fetch's page batch, including the two things
        -- that one learned the hard way: read the gate before firing, because a
        -- burst into a closed window is a burst spent, and arm it if the wave
        -- comes back limited rather than leaving the next caller to discover it.
        local token = #thin > 0 and get_token() or nil
        if token and Util.rate_cool() == 0 then
            local jobs, by_out = {}, {}
            for _, pl in ipairs(thin) do
                local tf = Util.tmpfile("plmeta")
                jobs[#jobs+1] = {url = Util.api_url("playlists/" .. pl.id,
                                     "fields=name,owner,images,tracks(total)"), out = tf}
                by_out[tf] = pl
            end
            local got = Util.curl_batch(jobs, {compressed = true, parallel = 6, timeout = 10,
                                               bodies = true,
                                               header = "Authorization: Bearer " .. token})
            local limited = false
            for _, j in ipairs(jobs) do
                local r = got and got[j.out]
                if r and r.code == "429" then limited = true end
                local full = (r and Util.is2xx(r.code)) and safe_decode(r.body) or nil
                os.remove(j.out)
                local pl = by_out[j.out]
                if pl and full and full.name and #full.name > 0 then
                    -- api_get would have collapsed available_markets on the way
                    -- out; a body read straight off the batch has to be told.
                    Util.mark_availability(full)
                    pl.name   = full.name
            ...
            if limited then Util.rate_arm() end
        end
...
-- How long an action card will wait for a lyrics lookup before drawing without
-- the answer. lrclib's get and search are 5s each at worst; ~2s is typical.
Util.LYRICS_WAIT = 6
local function resolve_lyrics_state(item)
    ...
    -- KEYED, so opening the same track's menu three times is one lookup rather
    -- than three: the pool refuses a second job under a key already in flight.
    Util.spawn_self({"--prefetch-lyrics", id, item.name or "", artist_names(item),
        (item.album and item.album.name) or "", dur}, nil, "lyrics:" .. id)
    -- THE CARD WAITS FOR THE ANSWER, and draws once, complete.
    --
    -- Round 6 made this not wait at all, and the card then drew a Lyrics row for
    -- a track that turned out to have none, and took it away two seconds later
    -- when the lookup landed. A card that rearranges itself under you is worse
    -- than one that arrives a moment later -- the UI breathes its loading glow
    -- over the wait, which is what the glow is for.
    --
    -- It waits on the JOB, not on a clock: the host says while the lookup is in
    -- flight (job_busy), so this stops the moment it lands, and the old 1.5s
    -- ceiling -- shorter than a typical lookup (~2s measured) -- is gone. What is
    -- left is a cap, for a lookup stuck behind lrclib's own timeouts. The memo is
    -- dropped before each look, because a 3s negative memo would otherwise keep
    -- answering "unknown" after the file has arrived.
    local key = "lyrics:" .. id
    local deadline = (Util.mono() or 0) + Util.LYRICS_WAIT
    repeat
        Util.wait(0.1)
        Util.lyr_bust(id)
        local st = Util.track_has_lyrics(id)
        if st ~= nil then return st end
        local busy = Util.host and Util.host.job_busy and Util.host.job_busy(key)
    until not busy or (Util.mono() or deadline) >= deadline
    -- ...AND IF THE CAP RAN OUT, the card draws with the answer still unknown and
    -- is redrawn when it lands: the host reports the job (`job-done`) and the UI
    -- is told to repaint the card. The rare case now, rather than every time.
    Util.lyr_bust(id)
    local st = Util.track_has_lyrics(id)
    if st == nil then Util.lyr_waiting = id end
    return st
end
...
-- `art_url`, tenth and optional, is where to fetch art_path from when the cover
-- is not on disk yet: the view hands over "" and the URL, the rows go out now, and
-- the picture follows (see Util.serve_named_cover). Before it existed an album
-- or a podcast had to download its cover BEFORE its track list could draw.
view_browse = function(entries, items, mesg, ctx, ctx_type, ctx_id, no_status, art_path, as_card, art_url)
...
browse_album = function(album_id, mesg)
    ...
    -- CACHE ONLY, with the URL beside it: a first visit draws its tracks at once
    -- and the sleeve arrives after them, the way the action menu's has since it
    -- stopped waiting on the CDN. This fetched first, so a new album sat there
    -- for up to four seconds with the engine unable to answer anything else.
    local url = ad.images and ad.images[1] and ad.images[1].url or nil
    local cover = url and Util.ensure_art_med(url, true) or nil
    view_browse(te, ad.tracks, mesg, "album", "album", album_id, nil, cover, nil, url)
    return true
end)
end
...
function Util.open_show(show_id, show_name)
    ...
    -- Cache only, for browse_album's reason: the list goes out now and the cover
    -- is fetched after it, through the URL handed to view_browse.
    local cover = url and (Util.ensure_art(Util.art_url(url, "1e02"), nil, {cached_only = true}) or "") or nil
...
view_art = function(item)
    ...
    elseif is_artist then
        ...
        art_path = Util.ensure_art(Util.art_url(imgs[1].url), "albums/high-res")
    end
...
view_actions = function(item, ctx_type, ctx_id, all_items, cidx, entries)
    ...
    while true do
        ...
        elseif key == "resume" then
            -- CHECKED, exactly as view_playback checks the same call. This menu
...
function Util.open_playlist(pl)
    ...
    -- THE PLAYLIST IS WHAT THIS PLACE IS A PICTURE OF. Double-clicking a backdrop
    -- ("art-here") opens whatever Util.serve_ctx_item names, and that is set only
    -- when the step that led here picked a row from a menu that handed over its
    -- `items` -- which Saved Albums does and the Playlists grid does not. So an
    -- album's sleeve opened the album and a playlist's cover opened the playing
    -- track instead. Named here, where the object is in hand, it holds for every
    -- way into a playlist rather than for the one menu that happened to pass it.
    Util.serve_ctx_item = pl
    ...
    while true do
        ...
        elseif asel == "Rename Playlist" then
            if not token then ui_say("No auth")
                ...
                if nn and nn ~= "" and nn ~= (pl.name or "") then
                    local url = Util.api_url("playlists/" .. pl.id)
        ...
        elseif asel == "Delete Playlist" then
            if not token then ui_say("No auth")
                ...
                if c == 1 then
                    local url = Util.api_url("playlists/" .. pl.id .. "/followers")
...
function Util.open_genre_tracks(genre)
    ...
    if not tracks then ui_say(Util.rate_why("No tracks for " .. genre)); return false end
...
function Util.open_recommendations(track_id, track_name)
    ...
    if not tracks then ui_say(Util.rate_why("No recommendations found")); return false end
...
function Util.open_search_results(query)
    ...
    -- NOT "No results", WHICH IS THE ONE THING THIS CANNOT MEAN.
    --
    -- api_search answers nil for every way a read can FAIL: the rate gate shut
    -- before the request left the process, a 401, a dead link, a 5xx that
    -- survived its retry. None of those are Spotify looking and finding nothing
    -- -- and "No results" is the sentence that claims the search WORKED, so it
    -- was the worst available thing to say. A search that really did match
    -- nothing does not come through here at all; it comes back as a table with
    -- empty pages, which is the branch below.
    --
    -- This is the exact complaint Util.rate_why was written for -- read its note,
    -- which lists "Failed to load playlist" said about a playlist that is
    -- perfectly fine. Every other read in this file already wraps its failure
    -- that way; search was the one that did not, and it also had the most
    -- misleading fallback of any of them.
    if not results then ui_say(Util.rate_why("Search failed")); return false end
    -- ...AND A SEARCH THAT GENUINELY MATCHED NOTHING SAYS SO.
    --
    -- With every page empty, format_search_results answers nil for whichever page
    -- opens, the loop below returns without calling view_browse, and the UI is
    -- handed an empty draw: no menu, no message, the previous list still on
    -- screen. A keypress that appears to do nothing at all -- which is the other
    -- half of "sometimes I get no results", and the half that says nothing.
    --
    -- Page 1 is All and spans every type in Util.SEARCH_TYPES (see SEARCH_PAGES),
    -- so one count answers it rather than six.
    if Util.search_page_count(results, 1) == 0 then
        ui_say("Nothing found for " .. query)
        return false
    end
...
function Util.open_category_playlists(category_id, category_name)
    ...
    if not pls then ui_say(Util.rate_why("No playlists")); return false end
...
function Util.browse_artist_albums(artist_id, artist_name)
    Util.scope({view="artist-albums", artist_id=artist_id, artist_name=artist_name or ""}, function()
        ...
        if not items then ui_say(Util.rate_why("No albums found")); return end
...
function Util.browse_related_artists(artist_id, artist_name)
    Util.scope({view="related", artist_id=artist_id, artist_name=artist_name or ""}, function()
        ...
        if not artists then ui_say(Util.rate_why("No related artists found")); return end
...
function Util.browse_top_by_artist(artist_id, artist_name)
    Util.scope({view="top-by-artist", artist_id=artist_id, artist_name=artist_name or ""}, function()
        ...
        if not tracks then ui_say(Util.rate_why("No top tracks found")); return end
...
view_lyrics = function(item)
    ...
    -- A song's words do not change, so an expired copy is simply the copy. This
    -- threw it away whenever the lookup failed -- and the lookup fails for the
    -- ordinary reason any request does -- leaving "No lyrics found" on a track
    -- whose lyrics were on disk the whole time.
    ...
    if timestamps then
        ...
        while true do
            ...
            local sel_line = ui_menu(display_lines,
                ...
                -- by_index: the ROW, not its words. See found_idx below.
                {prompt="Lyrics", mesg=mesg_base, by_index=true,
                 theme=THEME_LYR})
            if jump_to_track_pending then
                jump_to_track_pending = false
                goto lr_next
            ...
            -- THE LINE YOU PICKED, by position. It was found by matching the
            -- picked text against every line, first match wins: a chorus sung
            -- three times always jumped to its first time, and a line holding an
            -- `&`, `<` or `>` never matched at all, because the comparison
            -- escaped one side and not the other. The index is the row itself.
            local found_idx = type(sel_line) == "number" and sel_line or nil
                ...
                elseif not was_current then
                    ...
                    if token then
                        ...
                        local r = Util.api_write("PUT",
                            Util.api_url("me/player/play", dparam),
                            token, {timeout=3, body=body})
                        if Util.is2xx(r) then
                            P.recent_cmd_at = os.time()
...
view_add_pl = function(track_id, track_name)
    ...
    -- THIS MENU STAYS DRAWN, while its name field is up and after an add, and
    -- that is the whole of "create playlist says that playlist is no longer
    -- there".
    --
    -- It used to return the moment ui_ask raised the field, so the draw that
    -- answered the Create step was the track's action CARD underneath. The UI
    -- reads "a step that answered with a card" as never having been a place,
    -- lifts it off the trail onto the card's own hops -- and submitting the name
    -- then closes the card and those hops with it. The typed name was sent with
    -- the Create step gone, landed on this by_index menu as its ROW answer, and
    -- `ids["test"]` is nil.
    --
    -- Looping keeps this menu the one drawn: a redraw has the same identity as
    -- the draw that was just answered, so the replay does not feed it and the
    -- capture takes it. The UI then sees a place, holds the Create step while its
    -- field is up (see applyWhere's keep block), and after the add trims both
    -- steps -- so a refresh cannot replay "create test" a second time. Adding to
    -- an existing playlist lands here too, which is where the UI already ended
    -- up after a round trip through the card.
    while true do
    -- Rebuilt per pass: a playlist made on the previous pass belongs in the list.
    items = api_get_my_playlists() or items
    ...
    if ids[idx] == "__create__" then
        ...
        -- name, before the user had typed a character. Round the loop rather
        -- than out of it, so the field sits over this menu (see above).
        if pl_name and pl_name ~= "" then
            local cr = Util.create_playlist(pl_name, token, my_id)
            if cr then
                target_id = cr.id
                target_name = cr.name or pl_name
                bust_my_playlists(cr, true)
            else
                ui_say("Failed to create playlist")
            end
        end
    else
        target_id = ids[idx]
    ...
    if target_id then
        ...
        local add_url = Util.api_url("playlists/" .. target_id .. "/tracks")
...
local function view_playlists()
    ...
    while true do
        ...
        if idx == 1 then
            ...
            -- No `if me and me.id` wrapper any more: it made "we could not even
            -- find out who you are" a silent no-op, which from the outside looks
            -- exactly like a create that worked.
            local cr = Util.create_playlist(pl_name, token)
...
local function view_search()
    ...
    while true do
        ...
        --
        -- EACH ROW NAMED BY ITS QUERY, and this is the search bleed-through. A
        -- pick from this list went on the trail as its POSITION, and running a
        -- search moves its query to the top (Util.hist_add, below). So the next
        -- replay of the trail -- which Shift+Return, Tab and every redraw of the
        -- results all are -- took "row 2" again, found a different query there,
        -- and opened THAT search: Shift+Return on an Avalon Sutra result landed
        -- in the Joe Rogan results. Picking the top row only worked because the
        -- top row does not move.
        --
        -- `keys` is the channel that lets a replayed step prove which row it
        -- meant and follow it when it has moved (ui_menu's want_key guard).
        -- History holds each query once, so the query is a unique name. Not
        -- `items`: that switches on the track-list defaults -- Shift+Return
        -- opening an action menu, a middle click queueing -- for rows of words.
        local hkeys = {}
        for i, q in ipairs(hist) do hkeys[i] = q end
        local query = ui_menu(hist, {prompt="Search", keys=hkeys,
...
local function view_top_tracks()
    ...
    if not tracks then ui_say(Util.rate_why("No top tracks")); return end
...
function Util.open_podcast_search(query)
    local results = api_search(query)
    -- THE SAME TWO ANSWERS, TOLD APART. See Util.open_search_results: a nil from
    -- api_search is the read FAILING -- the rate gate, a 401, a dead link -- and
    -- an empty `shows` is Spotify answering that it has no podcasts by that name.
    -- Both said "No podcasts found", which is true of exactly one of them, and
    -- this is the path every one of the twenty-one topic tiles takes.
    if not results then
        ui_say(Util.rate_why("Podcast search failed"))
        return false
    end
    local shows = results.shows
...
function Util.shelf_tiles(list)
    ...
    for i, t in ipairs(list or {}) do
        ...
        -- A tile with no cover of its own wears the glyph named after its ROW KEY
        -- -- Util.ART_GLYPHS.playback, .collections, .podcasts, .search, .system.
        -- It wins over the fallback the row declares, so the artless rows can be
        -- given a mark of their own, and a row that grows a real cover later
        -- simply stops consulting either.
        --
        -- A TABLE LOOKUP WHERE A stat() STOOD. This used to build
        -- assets/<key>.png and ask the filesystem whether it existed, once per
        -- tile, on every draw of every grid -- 25 stats to draw the Podcasts grid,
        -- for an answer that is fixed at build time.
        local asset = Util.art_glyph(t.key)
...
local function view_your_queue()
    ...
    if not d then ui_say(Util.rate_why("Queue is empty")); return end
    local tracks = {}
...
Util.wait_playback_change = function(prev_id)
    ...
    Util.wait(0.15)
    -- THE PLAYER, ASKED DIRECTLY. The daemon files were meant to catch the
    -- change within a tick, and inside this call they cannot: the host posts its
    -- `daemon-snap` to the engine, and the engine is busy right here, so the snap
    -- waits behind the very request that caused it. Every pass therefore fell
    -- through to a me/player read -- four per skip, against the tightest limit
    -- Spotify has. Taking the snapshot ourselves is one local D-Bus read, and
    -- daemon_snap dedupes by track id, so the queued one that follows is a no-op.
    for _ = 1, 6 do
    ...
    -- The local player never moved: the music is somewhere else (another device)
    -- or spotifyd is not answering. Spotify is the only one who knows; ask once.
    last_playback = 0
    get_playback()
...
local function view_playback()
    ...
    while true do
        ...
        elseif key == "next" or key == "prev" then
            ...
            -- toast's buttons already skip. This went to the Web API -- a POST to
            -- me/player/next, then up to four me/player reads waiting to see the
            -- change -- which is five requests from the app's rate limit for
            -- something spotifyd does over D-Bus for free, and the one skip in
            -- the app that could fail with "rate limited".
            --
            -- The Web API is still the fallback, through recover_playback, for
            -- the case the local player cannot answer: spotifyd down or not on
            -- the bus.
            local prev_id = current_id
            local dir = key == "next" and 1 or -1
            local ok = Util.mpris{op = key == "next" and "next" or "previous"}.ok
            if ok then
                Util.playerctl_bust()
                P.recent_cmd_at = os.time()
                Util.wait_playback_change(prev_id)
                sync_queue_idx()
            elseif recover_playback(dir) then
                Util.wait_playback_change(prev_id)
                sync_queue_idx()
            else
                ui_say(Util.rate_why(key == "next" and "Failed to skip" or "Failed to go back"))
            end
...
function Util.view_listen_history()
    ...
    for _, e in ipairs(hist) do
        if type(e) == "table" and type(e.track) == "table" then
            ...
            local body, meta = display_track(e.track)
            entries[#entries+1] =
                Util.pack_meta(body .. SEP .. Util.ago(e.at), meta)
        end
    end
...
function Util.restart_daemons()
    ...
    ensure_spotifyd()
    Util.wait(3)
    -- THROUGH ensure_daemon, like the recorder on the next line, never a bare
    -- spawn. Embedded, spawn_self is a pool job -- and daemon_mode never returns,
    -- so every Restart (and every bitrate change, which restarts) parked a second
    -- MPRIS watcher on one of the pool's three threads for the rest of the
    -- session. Three of them and nothing else could run: no lyrics prefetch, no
    -- cover batches, no library refresh. The host already watches the player;
    -- ensure_daemon knows that, and only spawns when there is no host.
    Util.ensure_daemon()
...
function Util.ui_pick(spec)
    ...
    while true do
        ...
        local sel = ui_menu(rows, {prompt = spec.label,
            ...
            context = true, sticky = true,
            -- THE PANEL IS THE PREVIEW. Every card dims and blurs the list it
            -- floats over, and for this one setting that veil hid the very thing
            -- being chosen: the ground went from 80% to 50% under a 45% scrim and
            -- barely moved, then jumped when the card closed. Unveiled, each value
            -- shows on the panel the moment it is picked.
            unveil = (spec.key == "opacity") or nil,
...
local function view_system()
    ...
    while true do
        ...
        elseif clean == "Quit" then
            ...
            -- safe against. The host already ends everything on the `quit`
            -- event: it stops this thread, the serve loop runs dry and returns.
            -- So the cleanup happens FIRST -- it can make a request, and the
            -- host gives this thread two seconds once it starts stopping --
            -- then the host is told, and this returns like any other row.
            if Util.hosted then
                Util.clean_up()
...
-- Tab's card (Trail Steps / Trail History): see lib/trail.lua. ui_menu and
-- ui_say go in as wrappers because serve mode reassigns both after this line.
require("lib.trail")(Util, {json = json, replay_session = replay_session, view_label = view_label,
                            ui_menu = function(...) return ui_menu(...) end,
...
-- For Util.restart_daemons, which is defined thousands of lines above this and
-- would otherwise compile the name as a global.
Util.ensure_daemon = ensure_daemon
...
local function daemon_mode()
    ...
    if not claimed then
        ...
        claimed = trim(shell("mkdir " .. shell_quote(claim) .. " 2>/dev/null && echo ok") or "") == "ok"
    end
...
function Util.run_notify()
    ...
    if id and id:match("^[A-Za-z0-9]+$") then
        ...
        -- ALREADY IN THE LIBRARY, usually: what plays is mostly what is saved,
        -- and Util.track_from_cache holds hundreds of complete track objects --
        -- album id, explicit flag and all. Util.daemon_snap already used that
        -- copy for now_track.json, so fetching tracks/{id} here bought nothing
        -- but one request per song change. Episodes have no such cache.
        local cached = (not is_ep) and Util.track_from_cache(id) or nil
        if cached and cached.album and cached.album.id then
            track = cached
        else
            track = api_get((is_ep and "episodes/" or "tracks/") .. id, Util.with_market())
        end
        if track and track ~= cached then
...
-- WHICH OF THOSE MAY FETCH FOR THEMSELVES, and which read a cache something else
-- is already committed to filling.
--
-- Collections and Podcasts are Spotify's editorial shelves and nothing else in
-- the app reads them, so their warm IS what populates those caches and has to go
-- to the network. `main` costs one me request and is likewise nobody else's.
--
-- THE LIBRARY IS THE EXCEPTION, and it was the largest burst in the file run
-- twice. Util.LIBRARY_ROWS resolves its covers through load_liked_tracks,
-- load_saved_albums and load_followed_artists -- exactly the three lists
-- Util.REVALIDATORS.library rebuilds -- and init_library spawns both jobs in the
-- same instant. On a fresh cache this one walked me/tracks sequentially, fifty at
-- a time, while parallel_fetch_library walked the identical endpoint four pages
-- at a time: ~35 pages of me/tracks asked for twice, concurrently, on the one
-- launch with nothing cached to answer from. That is the first-run 429.
--
-- So this kind warms COVERS ONLY, under Util.cache_only: a shelf already on disk
-- gets its artwork, and one that is not yet there is left to the job that owns
-- it. Nothing is lost -- Util.shelf_tiles reports the grid cold on its first
-- open and spawns this again (see Util.view_tile_grid), by which time the
-- library job has landed and there is something to warm.
Util.SHELF_COVERS_ONLY = {library = true}
...
function Util.run_shelf_warm()
    ...
    local list = {}
    -- See Util.SHELF_COVERS_ONLY. Saved and restored rather than set and cleared,
    -- because Util.shelf_tiles does the same around its own read and a warm that
    -- left the flag on would silence every fetch after it in this job.
    local was_cache_only = Util.cache_only
    if Util.SHELF_COVERS_ONLY[kind] then Util.cache_only = true end
    ...
    -- Back before the covers: those are i.scdn.co fetches rather than API reads,
    -- so the flag means nothing to them -- but the restore belongs with the loop
    -- it guarded, not at the end of a function that exits.
    Util.cache_only = was_cache_only
    if #list > 0 then
        Util._art_batch(list)
...
-- THE one place that knows how a decorated row is packed: four copies of this
-- pattern had grown across the serve layer, and a change to the packing would
-- have had to find them all.
--
-- Two side-channels: "\0icon\x1f<path>" from album_thumbs, and "\0meta\x1f<col>"
-- from the track formatters, where <col> is itself "<marks>\x1f<duration>" --
-- split by serve_rows into the two cells the right-hand column draws. Peeled
-- independently, so neither channel has to know whether the other came first.
-- Qt has no use for any of the packing, so rows leave here as fields.
-- THE PACK HALF, for the row builders. A row with neither a duration nor a mark
-- carries no channel at all rather than an empty one, so a list with no such
-- column costs nothing to send.
function Util.pack_meta(text, meta)
    if not meta or meta == "" or meta == "\x1f" then return text end
    return text .. "\0meta\x1f" .. meta
end

function Util.serve_unpack(entry)
    if type(entry) ~= "string" then return nil, nil, nil end
    local text, meta = entry, nil
    local head, m, tail = text:match("^(.-)\0meta\x1f([^\0]*)(.*)$")
    if head then text, meta = head .. tail, m end
    local t2, icon = text:match("^(.-)\0icon\x1f(.*)$")
    if icon then text = t2 end
...
function Util.serve_rows(entries, items, keys)
    ...
    for i, e in ipairs(entries or {}) do
        ...
        --
        -- `meta` is the right-hand column -- the track's length and what is true
        -- about it. Sent apart from the label because it is DRAWN apart: written
        -- into the line it would either push every title to a different column or
        -- elide away with a long one. See display_track.
        -- Two cells, not one string: the clock is flush right and the marks sit
        -- just inside it, so the durations line up down the list however many
        -- marks a row carries.
        local mk, dur
...
Util.SERVE_VIEWS = {
    ...
    -- A SAVED PLACE, REOPENED FROM ITS STACK: a menu from Trail History. The same
    -- move `origin` makes for one entry, for a whole path -- install the stack,
    -- let replay_session open it -- and the path the UI sends after this hop
    -- walks on from there like any other view. The UI shortens `restore` by one
    -- entry per Backspace, which is what walks you back up the path.
    restore          = function(a)
        local st = a and a.restore
        if type(st) ~= "table" or #st == 0 then ui_say("Nothing to reopen"); return end
        Util.session_set(json.decode(json.encode(st)))
        replay_session()
    end,
...
function Util.serve_draw(name, d)
    ...
    return {
        ...
        unveil = o.unveil and true or nil,
...
function Util.serve_nav(args)
    ...
    local function fold(segs, list, upto)
        for i = 1, upto do
            ...
            if type(h) == "table" and h.cmd then
                ...
                --
                -- `buf` rides along the same way, and says the opposite thing
                -- about this segment: it is a CARD -- see the crumb loop below and
                -- main.qml's openCard.
                segs[#segs + 1] = {cmd = h.cmd, key = h.key, mode = h.mode,
                                   buf = h.buf, restore = h.restore, path = {}}
            elseif #segs > 0 then
    ...
    for i = 1, #segs do
        ...
        --
        -- ...AND NO FETCHES EITHER, which is the larger half of that sentence and
        -- was the missing one. A pick is replayed against the WHOLE trail -- see
        -- the note above this function -- so playing a track five levels down
        -- re-runs every menu above it, and a menu whose cache had lapsed in the
        -- meantime went to the network, synchronously, for rows that are then
        -- thrown away. That is the wait on Return: not the play, which is one
        -- write, but up to four list fetches queued in front of it for menus
        -- nobody is going to see. Worse when the gate is shut, because then those
        -- fetches answer nil and the replay fails ON A MENU YOU ARE NOT IN --
        -- which is the shape of "sometimes it just does nothing".
        --
        -- A walked-through segment needs exactly enough to turn its step index
        -- into the next hop, and whatever it last knew answers that: the row order
        -- of a list you walked through a moment ago is not in question.
        -- Util.cache_only is the same read Util.shelf_tiles takes for the same
        -- reason, and cached_fetch falls back under it to a copy with no TTL at
        -- all -- so a segment that has ever been drawn still answers, and one that
        -- has not cannot exist: walking into a menu is what caches it, and
        -- nav.json lives in the same cache directory, so a trail and the caches it
        -- refers to are wiped together or not at all.
        --
        -- The LAST segment is the menu being drawn and keeps full fetch rights.
        -- Saved and restored rather than set and cleared, matching shelf_tiles.
        Util.serve_art_skip = not last
        local was_cache_only = Util.cache_only
        Util.cache_only = not last
        local ok, res = pcall(function()
            ...
            elseif sg.cmd == "view" then
                return Util.serve_view({name = sg.key, path = sg.path,
                                        mode = sg.mode, restore = sg.restore,
                                        keepCtx = (i >= tail_from) and keep_ctx or nil,
        ...
        draw = res
        -- A TAIL SEGMENT THAT DREW A CARD contributes nothing to the crumb,
        -- because a card is not a step you took. ONE THAT DREW A PLACE IS WHERE
        -- YOU ARE, and has to name it.
        --
        -- Skipping every tail segment was right while a tail could only ever be a
        -- card. It cannot: the search box is a card whose step opens the RESULTS,
        -- and the results arrive on this very draw -- still on the tail, because
        -- the UI does not adopt them onto the trail until it has this answer in
        -- hand. So the menu being drawn was handed the crumb of the list it was
        -- opened over, or -- from Main, with nothing on the trail -- no crumb at
        -- all. An empty crumb collapses the breadcrumb (it needs more than one
        -- part to be worth drawing) and the message bar collapses with it, which
        -- is the top of the panel disappearing on search results.
        --
        -- The draw says which it is. `context` is set by Util.serve_draw from the
        -- menu's own flag, so this asks the thing that knows rather than guessing
        -- from the hop -- the same distinction the UI draws with isContext, and
        -- the reason a card and the results it leads to are not one entity.
        if i >= tail_from and (type(res) ~= "table" or res.context or res.empty) then
            goto next_seg
        end
        ...
        if #chain > 0 then
            -- `buf` IS ABOUT A HOP, NOT ABOUT THIS SEGMENT, and this is where
            -- confusing the two did real damage.
            --
            -- A segment named by a card hop draws the CARD only while its path is
            -- empty -- and a segment with an empty path is one nobody picked
            -- anything in, which lives on the tail and never reaches this loop. By
            -- the time a card hop is on the TRAIL it has a step after it, and what
            -- the segment draws is wherever that step went: the search results, the
            -- album, the artist. That is a place, and it gets a place's crumb.
            --
            -- Giving it a buffer's treatment here took the crumb parts off the very
            -- menu the card led to -- so search results arrived with no trail at
            -- all, and the message bar collapsed with them. The card and the
            -- results are not one entity; the hop is the route to the second, not
            -- the first.
            --
            -- Where a card must not appear is under the CURSOR, and that is the
            -- UI's to enforce: see skipCtx, which walks past a `buf` hop in both
            -- directions so backing out of the results returns to the list rather
            -- than redrawing the box.
...
function Util.serve_nav_save(hops, pos, tip, tip_roots)
    ...
    --
    -- A CARD IS THE THIRD OF THESE, and the general case the other two are
    -- instances of: a trail may not END on something that is not a place.
    --
    -- The UI marks one with `buf` (see main.qml's openCard) and already refuses to
    -- leave the cursor on one -- skipCtx walks past them in both directions and
    -- applyWhere unmarks a buffer that turned out to draw a real menu. This is the
    -- same rule written where it survives a restart: a nav.json whose last hop is
    -- a card would be replayed on the next cold start, the segment would answer
    -- `context`, and spoot would open onto an action menu floating over nothing,
    -- with no list behind it to close onto. It is also what heals a trail written
    -- by a build that did not know any of this.
    --
    -- AND NOTHING BEYOND ONE EITHER. A portal is not remembered, so neither is
    -- the far side of it: a trail of "Liked Tracks, then through the search box,
    -- then these results" resumes as "Liked Tracks", because the two steps after
    -- it only exist as directions THROUGH a thing that is not a place. Resuming
    -- into them would mean replaying the card to get there, which is the card
    -- being remembered however the crumb chooses to draw it.
    --
    -- Trimmed from the END, not cut at the first one. Cutting at the first buffer
    -- threw away everything BEYOND a card as well -- so a trail of "Liked Tracks,
    -- then through the search box, then these results" resumed as "Liked Tracks",
    -- and the results, which are an ordinary place, were not remembered at all.
    -- What must not be resumed into is a card, and a card is only ever the LAST
    -- hop: with a step after it, the segment draws whatever that step reached.
    hops = hops or {}
    local function not_a_place(h)
        if type(h) ~= "table" then return false end
        if h.buf then return true end
        return h.cmd == "view" and h.key == "listen"
    end
    local n = #hops
    while n > 0 and not_a_place(hops[n]) do n = n - 1 end
...
function Util.serve_draw_cover(d)
    ...
    if not Util.serve_shelf(d) then
        ...
        if Util.serve_ctx_url then
            ...
            -- ...OR THE SMALL COPY OF THE SAME COVER, which is nearly always here.
            -- A container names its cover as a URL when the 640px file is not on
            -- disk (see browse_album), and the draw then shipped NO cover -- so
            -- the UI went on wearing the last backdrop it had until the fetch
            -- landed. Opening albums one after another from an artist's grid
            -- flashed the album you started from, every time. The grid you
            -- clicked in drew this very cover at 300px, so it goes out now and
            -- the 640px one replaces it when the after-reply pass brings it.
            local small = Util.ensure_art(Util.art_url(Util.serve_ctx_url, "1e02"), nil,
                                     {cached_only = true})
            if small and small ~= "" then return small end
        end
        return nil
    end
...
function Util.item_cover(item)
    ...
    -- A PLACEHOLDER IS NOT A PICTURE OF ANYTHING. album_thumbs answers with
    -- Util.ART_NONE for a row that has no cover, and this is the one funnel both
    -- the list backdrop and a card's cover read -- so without this the sentinel
    -- would arrive at an Image as a source it cannot load, and a 400px blow-up of
    -- a placeholder glyph is not what either of them is for. nil is what the
    -- note below already says every branch here should answer.
    if Util.is_art_glyph(icon) then return nil end
    return icon
end
...
function Util.serve_playback()
    ...
    return {
        ...
        -- ...AND THE OTHER TWO STATUSES, AS FACTS RATHER THAN AS GLYPHS.
        --
        -- `icons` below stays exactly as it was and is still what the dock and
        -- every row wear -- one finished string, the engine's answer about how a
        -- status looks. It cannot be what the now-playing strip wears any more,
        -- for the reason `liked` was already split out above it: a glyph in a
        -- string is a picture, and these three have become CONTROLS. The heart is
        -- pressed to save and unsave, the lyrics mark opens the lyrics, and the
        -- explicit mark is the one of the three that is neither -- it is a fact
        -- about the track, so it is the one that is only ever coloured.
        --
        -- The lyrics state is a word rather than two booleans because it has
        -- three values and Util.status_icons already distinguishes them: no
        -- lyrics at all, plain ones, or synced. The UI picks the glyph and the
        -- engine names the state, which is the same split `repeat_` uses.
        explicit = (t and t.explicit) or false,
        lyrics   = (function()
            if not (t and t.id) then return "none" end
            if not Util.has_lyrics(t.id) then return "none" end
            return Util.has_synced_lyrics(t.id) and "synced" or "plain"
        end)(),
...
Util.SERVE = {
    ...
    -- A KEYED BACKGROUND JOB FINISHED, reported by the host's JobPool. Only a
    -- lyrics answer means anything here yet: a lookup (`lyrics:`) or the track
    -- change helper, which looks lyrics up too (`notify:`). The memo is dropped
    -- so the next read sees the file the job wrote, and if the open action card
    ...
    ["job-done"] = function(a)
        ...
        if id then
            Util.lyr_bust(id)
            if Util.lyr_waiting == id then
                Util.lyr_waiting = nil
                Util.serve_write({ev = "lyrics-known", id = id})
            end
        end
        return {ok = true}
    end,
        ...
        if Util.serve_capture then
            ...
            if Util.alt_pressed and ans ~= nil and not opts.alt_select
               and type(opts.items) == "table" and type(ans) == "number" then
                ...
                if item then
                    Util.alt_pressed = false
                    -- DID THE CARD ACTUALLY DRAW? `return nil` here assumed it
                    -- always does -- "the action menu was the destination" -- and
                    -- unwinds this menu on that assumption. Usually harmless: the
                    -- card captured first, cap[1] is the card, and the list this
                    -- was raised from is never asked for.
                    --
                    -- But view_actions has rows that answer and CLOSE without
                    -- drawing anything: Unlike removes the row the card is about,
                    -- and Go to Album from inside that same album is already
                    -- there. Nothing was captured, so unwinding handed the UI the
                    -- first draw it could find -- the menu BEHIND this one. That
                    -- is "unliking works but also goes back a page", and Go to
                    -- Album did it too, from every album.
                    --
                    -- So it is asked rather than assumed. A card that drew is
                    -- still the destination; one that closed leaves us standing
                    -- where we were, and spending the answer -- ans = nil -- makes
                    -- the capture below redraw exactly that. Same idiom as the
                    -- Delete and Queue branches above, and the capture runs
                    -- opts.refresh itself, so the redraw is the list WITHOUT the
                    -- row that was just unliked.
                    local drawn = #Util.serve_capture
                    view_actions(item, opts.ctx_type, opts.ctx_id, opts.items, ans, opts.entries)
                    if #Util.serve_capture > drawn then return nil end
                    ans = nil
                end
            end
...
elseif arg and (arg[1] == "--help" or arg[1] == "-h") then
    -- It did not exist, and `--client-id` sent people to it: an unknown flag
    -- exits 2 with nothing said. The flags a person types, then the switches
    -- that are otherwise only documented at the one line that reads them.
    print([[usage: spoot [option]

  (no option)           open spoot, or summon the one already running
  --listen              open straight on Listen (identify what is playing)
  --client-id [id]      show which Spotify app id spoot uses, or switch to
                        your own (an empty id goes back to the shared one).
                        Your own id has a rate limit nobody else spends, but
                        apps made after Nov 2024 cannot reach Categories,
                        Made For You, Featured, Discover by Genre or Related
                        Artists. Switching logs you out once.
  --doctor              say what is still owed: the login, the playback device
  --thumb-report [n]    why grid covers came up empty, last n draws (20)
  --help                this

environment
  SPOOT_DEV=1             reload the QML when a file under ui/ changes
  SPOOT_REQLOG=<file>     append every request spoot makes to <file>
  SPOOT_FORCE_CURL=1      send requests through curl, not Qt
  SPOOT_FORCE_PLAYERCTL=1 talk to the player through playerctl, not D-Bus
  SPOOT_NOTIFY_BR=1       use <br/> for line breaks in notifications
  SPOOT_NO_AUTOSETUP=1    skip the first-run login flow
  SPOOT_DOCK_DEBUG=1      log how the dock measured its screen edge
  SPOOT_DEBUG_SCREENS=1   log every monitor and which one spoot opened on

The remaining --flags (--serve, --daemon, --revalidate, --prefetch-*, ...) are
spoot's own background entry points and are not meant to be run by hand.]])
    os.exit(0)
