...
Util.UI_SETTINGS = {
    ...
    {key = "opacity", default = 80, kind = "range", min = 50, max = 100, step = 5,
     label = "Opacity", unit = "%", why = "how solid the panel's ground is"},
    {key = "shadows", default = true, kind = "bool", label = "Shadows",
     why = "the drop shadow under the panel and its cards"},
...
local function api_get(path, params, _retry)
    ...
    if status == 429 then
        local secs = string.match(r.headers or "", "[Rr]etry%-[Aa]fter:%s*(%d+)") or "30"
...
view_volume = function()
    ...
    while true do
        ...
        elseif vi == "Volume -5" then
            ...
            if v then
                Util.mpris{op = "setvol", value = v / 100}
                mem_bust("_playerctl_vol")
                save_volume(v)
                disp_vol = v
            end
...
function Util.ui_pick(spec)
    ...
    while true do
        ...
        -- A LIST, INCLUDING FOR THE ANCHOR, which was meant to be a 3x3. A grid
        -- here is a grid of COVER tiles -- every grid view in spoot draws
        -- artwork, and nine of them with no artwork would be nine empty frames
        -- with words under them. The nine positions are listed in reading order
        -- instead, which says the same thing and draws correctly. A real 3x3 of
        -- text cells is a new delegate, not an option on this call.
        local sel = rofi_dmenu(rows, {prompt = spec.label,
...
Util.view_ui_settings = function()
    ...
    while true do
        ...
        for i, spec in ipairs(Util.UI_SETTINGS) do
            rows[i] = spec.label .. SEP .. Util.ui_show(spec, cur[spec.key])
        end
        local sel = rofi_dmenu(rows, {prompt = "UI Settings",
...
function Util.setup_state()
    -- NOT curl. The login has made its requests through the host's own network
    -- stack since the transport work, so listing it here would block a sign-in
    -- over a program the sign-in does not use.
    local names = {"openssl", "perl", "xdg-open"}
    local lack = {}
...
function Util.recent_watch_mode()
    ...
    while true do
        ...
        if cooldown and os.time() < cooldown then
            Util.wait(math.max(cooldown - os.time(), 1))
        else
...
Util.SERVE = {
    ...
    ["recent-tick"] = function()
        ...
        local cooldown = tonumber((read_file(P.rate_cooldown) or ""):match("%d+"))
        if cooldown and os.time() < cooldown then return {skipped = true} end
