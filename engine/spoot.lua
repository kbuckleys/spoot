        ...
        if os.time() > data.expires_at - 120 then
            ...
            if data.refresh_token and type(data.refresh_token) == "string" then
                ...
                if rd and rd.access_token then
                    ...
                    local wrote = write_file(P.token, json.encode(data))
                    if not wrote then wrote = write_file(P.token, json.encode(data)) end
...
local function do_playback_cmd(cmd)
    ...
    if not token then return nil end
    local device_id = get_spotifyd_device()
    local url = Util.api_url("me/player/" .. cmd)
        .. (device_id and ("?device_id=" .. device_id) or "")
    ...
    return cached_fetch("category_playlists_" .. cat_id, P.mass .. "/category_playlists_" .. cat_id .. ".json", CACHE_TTL_MED, function()
        ...
        if token and Util.rate_cool() == 0 then
            ...
            for _, j in ipairs(jobs) do
                ...
                local full = (r and Util.is2xx(r.code)) and safe_decode(read_file(j.out)) or nil
                os.remove(j.out)
...
view_add_pl = function(track_id, track_name)
    ...
    if ids[idx] == "__create__" then
        ...
        if not cr then ui_say("Failed to create playlist"); return end
        target_id = cr.id
...
-- in a fourth.
--
-- A SHUT GATE IS NOT A REASON NOT TO RUN. This answered true for any cooldown at
-- all and main() then quit on it, so launching inside a fifteen-second window
-- printed a sentence and exited -- with a full cache on disk and a menu that
-- needs no request to draw. Measured: opening Main and walking four shelves warm
-- costs one request, and the playback poll costs none while spotifyd is up. On a
-- shared client id the gate is shut a good fraction of the time, which made this
-- "spoot won't open" rather than "spoot is a few seconds stale".
--
-- So it stands down only when there is also nothing to show: no library on disk
-- means the menu behind this would be empty anyway, and saying why beats drawing
-- an empty shelf. Same cache_exists test init_library uses for the same reason.
local function check_rate_cooldown()
    local left = Util.rate_cool()
    if left <= 0 then return false end
    if cache_exists(P.liked) or cache_exists(P.albums) or cache_exists(P.artists) then
        -- Cache to draw from: say it once, quietly, and open.
        ui_say(Util.rate_why("Spotify API rate limit active"))
        return false
    end
...
local function init_library(cold_start)
    ...
    -- ABSENT, NOT MERELY EXPIRED, and the difference is the whole point of the
    -- branch below. P.ttl is fifteen minutes, so the test above is true on very
    -- nearly every launch -- and what it starts then is a fingerprint probe:
    -- three ~1 KB requests that usually end in Util.cache_touch and no paging at
    -- all. Deferring anything for THAT would be deferring it always.
    --
    -- A missing file is the other thing entirely: no fingerprint to compare, so
    -- every probe differs from nil and Util.rebuild_library re-pages the lot.
    -- That is the launch this is about, and it happens once per install.
    local cold_cache = not (cache_exists(P.liked) and cache_exists(P.albums)
                            and cache_exists(P.artists))
    populate_liked_ids()
    -- ONE SWEEP AT A TIME ON A COLD CACHE, and this is the whole of the first-run
    -- 429.
    --
    -- The three spawns here are each reasonable and were written apart, so
    -- nothing in the file ever added them up. Together, on a launch with an empty
    -- cache, they are: ~35 pages of me/tracks and me/albums four at a time (the
    -- library build), a walk of every owned playlist's tracks six pages at a time
    -- (the index), twenty-one /search queries (the podcast warm) and three more
    -- library walks (the library warm, before Util.SHELF_COVERS_ONLY). JobPool
    -- runs three of those jobs at once, so the burst is 12-24 requests in flight
    -- within a second of start-up, against an account with no cache to fall back
    -- on -- and the gate, being read once per batch, is open for all of them.
    --
    -- WHICH ONE ACTUALLY MATTERS IS NOT IN DOUBT. The library is what every menu
    -- in the app reads; an index that only decides whether one action-menu row
    -- appears, and cover art for a grid nobody has walked to yet, are the two
    -- things that can wait a minute. So while the library is being built they do:
    -- both re-spawn by themselves the moment something wants them -- Util.pl_index
    -- spawns the index when the action menu finds none, and Util.shelf_tiles
    -- reports a grid cold on its first open and spawns its warm (see
    -- Util.view_tile_grid) -- which is the same lazy path they take on every
    -- launch after this one.
    --
    -- EVERY OTHER LAUNCH IS UNCHANGED, which is all but the first: with the three
    -- caches on disk -- however old -- both spawns happen exactly where they
    -- always did.
    if not cold_cache then
