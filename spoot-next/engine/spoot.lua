...
-- already passes to rofi_dmenu, and what the UI keys its geometry table on -- so
...
local THEME_MENU, THEME_LYR, THEME_MSG, THEME_SUB, THEME_BINDS, THEME_META, THEME_ART =
      "menu", "lyrics", "message", "sub", "binds", "meta", "art"
...
Util.THEME_IMP     = "imp"         -- artist impression: 640, where a cover is 1000
Util.THEME_LISTEN  = "listen"
...
local function rofi_dmenu(entries, opts)
    error("rofi_dmenu called before serve mode replaced it", 2)
end
...
rofi_message = function(msg, theme)
    ...
    error("rofi_message called before serve mode replaced it", 2)
end
...
local function rofi_input(prompt, preset, theme)
    ...
    error("rofi_input called before serve mode replaced it", 2)
end
...
Util.write_art_theme = function(name, art_path, art_url)
...
local function api_get(path, params, _retry)
    ...
    if status == 429 then
        ...
        local cool = tonumber((read_file(P.rate_cooldown) or ""):match("%d+"))
        if not Util.detached then
            Util.secure_write(P.rate_cooldown, os.time() + math.min(tonumber(secs), 5) + 5)
        end
...
function Util.clean_exit(code)
    ...
    -- A `rm -f /tmp/spoot_theme_*.rasi` stood here, sweeping up the per-call
    -- themes Util.write_art_theme used to generate. It writes no file now, so
    -- the sweep had nothing left to find -- and it was a fork on every exit to
    -- go and look.
...
view_art = function(item)
    ...
    Util.serve_write({ev = "art-view", path = art_path,
                      ...
                      -- imp.rasi for an artist impression (640px), art.rasi for
                      -- a cover (1000px) -- the same split view_art always made
                      -- when it picked between Util.THEME_IMP and THEME_ART.
                      theme = is_artist and "imp" or "art",
...
view_actions = function(item, ctx_type, ctx_id, all_items, cidx, entries)
    ...
    -- Names the theme AND records the cover, which is why the call survives its
    -- own name: it used to write a per-call .rasi baking the art in as the
    -- window's background-image, and the file it wrote was what the two locals
    -- here held and every early return had to unlink. Qt draws the cover
    -- directly, so there is no file -- but there were still seven os.remove
    -- calls unlinking the string "action" from whatever directory the engine
    -- happened to be started in.
    local action_theme = Util.write_art_theme("action", art_path, art_url)
    ...
    while true do
        ...
        elseif key == "Remove from Playlist" then
            ...
            elseif #rm_targets > 1 then
                ...
                local pick = rofi_dmenu(names, {prompt="Remove from", mesg=(item.name or "") .. SEP .. #rm_targets .. " playlists", by_index=true, theme=THEME_SUB})
...
function Util.search_page_pick(results, page)
    ...
    local idx = rofi_dmenu(rows, {prompt="Type", mesg="Filter results by type", by_index=true, theme=THEME_SUB})
    ...
    while true do
        ...
        elseif asel == "Delete Playlist" then
            if not token then rofi_message("No auth")
                ...
                local c = rofi_dmenu({"DELETE","Cancel"}, {prompt="Delete", mesg="Delete " .. (pl.name or "") .. "?", by_index=true, theme=THEME_SUB, confirm=true})
                if c == 1 then
...
function Util.browse_artist_albums(artist_id, artist_name)
    Util.scope({view="artist-albums", artist_id=artist_id, artist_name=artist_name or ""}, function()
        ...
        while true do
            ...
            local aidx = rofi_dmenu(ae, {prompt=artist_name or "", mesg=mesg, by_index=true, thumbs=true, alt_select=true,
...
function Util.browse_related_artists(artist_id, artist_name)
    Util.scope({view="related", artist_id=artist_id, artist_name=artist_name or ""}, function()
        ...
        while true do
            ...
            local ridx = rofi_dmenu(ae, {prompt="Related to " .. (artist_name or ""), mesg=mesg,
                                         by_index=true,
...
view_add_pl = function(track_id, track_name)
    ...
    local idx = rofi_dmenu(names, {prompt="Add to Playlist", mesg="Select a playlist", by_index=true})
    if not idx then return end
...
local function view_playlists()
    ...
    while true do
        ...
        local idx = rofi_dmenu(entries, {prompt="Playlists", mesg="Playlists" .. SEP .. #pls, by_index=true, alt_select=true,
            thumbs=true,
...
local function view_categories()
    ...
    while true do
        ...
        local idx = rofi_dmenu(ce, {prompt="Categories", mesg="Categories" .. SEP .. #cats, by_index=true,
            thumbs=true,
...
function Util.view_tile_grid(spec)
    ...
    while true do
        ...
        local idx = rofi_dmenu(entries, {prompt=spec.prompt, mesg=spec.prompt, by_index=true,
...
view_new_releases = function()
    ...
    while true do
        ...
        local idx = rofi_dmenu(entries, {prompt="New Releases", mesg="New Releases" .. SEP .. #albums .. " albums", by_index=true, thumbs=true, alt_select=true,
...
view_volume = function()
    ...
    while true do
        ...
        local vi = rofi_dmenu(vol_acts,
            {prompt="Volume", mesg=vol_mesg(disp_vol), theme=THEME_SUB})
...
view_seek = function(item)
    ...
    while true do
        ...
        local si = rofi_dmenu(seeks, {prompt="Seek", theme=THEME_SUB})
        if not si then
            if jump_to_track_pending then return end
            break
        end
...
local function view_playback()
    ...
    -- first and the picture follows. See Util.write_art_theme.
    ...
    Util.write_art_theme("sub", Util.ensure_art_med(pb_art, true), pb_art)
    while true do
        build_items()
...
Util.view_bitrate = function()
    ...
    while true do
        ...
        if n and n ~= cur then
            ...
            local c = rofi_dmenu({"Restart", "Abort"},
                {prompt="Bitrate", mesg=n .. " kbps" .. SEP .. "restart daemons to apply?", by_index=true, theme=THEME_SUB, confirm=true})
            if c == 1 then
        ...
        if sel == 3 then
            if not sz then
                ...
                local c = rofi_dmenu({"Clear Cache", "Abort"},
                    {prompt="Track Cache", mesg="delete " .. sz .. " of cached tracks?", by_index=true, theme=THEME_SUB, confirm=true})
                if c == 1 then
            ...
            if want ~= on then
                local c = rofi_dmenu({"Restart", "Abort"},
                    {prompt="Track Cache",
                     mesg=Util.track_cache_label(want) .. SEP .. "restart daemons to apply?", by_index=true, theme=THEME_SUB, confirm=true})
                if c == 1 then
                    -- Before the restart: ensure_spotifyd reads the saved value.
...
local function view_system()
    ...
    local keys = {"keybinds", "volume", "bitrate", "trackcache", "trailjump",
                  "clearsession", "refresh", "reauth", "deviceauth", "restart", "kill"}
    Util.scope({view="system"}, function()
    local sys_key = "system:"
    local pre_sel = Util.pos_row(sys_key, keys)
    while true do
        local sel = rofi_dmenu(items, {prompt="System", theme=THEME_SUB})
        ...
        if clean == "Keybinds" then
            -- Structured, so the front end can lay out two real columns and size
            -- itself to the content. rofi got a padded string because it could
            -- do neither.
        ...
        elseif clean == "Authorise Playback" then
            ...
            if Util.device_auth() then
                Util.restart_daemons()
                rofi_message("Playback device authorised")
            else
                rofi_message("Could not authorise the playback device")
            end
...
-- ============================================================================
--
-- spoot is a program that runs other programs: curl carries every request,
-- playerctl carries every transport key, spotifyd is the thing actually making
-- sound. Missing any of them fails somewhere deep and quiet -- a menu that never
-- fills, a play key that does nothing -- so they are checked once at startup and
-- named plainly.
--
-- The list is derived from what this file actually shells out to. Anything added
-- to it here must be a program some line above genuinely runs, or this becomes a
-- second, wrong description of the same thing.
--
-- `pkg` lists the names a program is packaged under, best first, since a binary
-- and its package are frequently not called the same thing (wl-copy lives in
-- wl-clipboard; notify-send in libnotify, libnotify-bin or libnotify-tools
-- depending on whose repo you ask). A per-manager entry overrides `default`.
--
-- WHICH OF THEM ACTUALLY EXISTS IS ASKED, NOT ASSUMED -- see pkg_exists. This
-- used to be a table of my guesses about who packages what, with `false`
-- standing in for "I think this one is AUR-only", and it was wrong about Arch on
-- its first outing: spotifyd, librespot and songrec are all in extra. Guessing
-- costs a user the automatic install they should have had, and guessing the
-- other way names a package that does not exist -- which fails the whole batch
-- and takes the packages that DID exist down with it. So the manager is asked.
Util.DEPS = {
    {bin = "curl", kind = "required",
     why = "every request to Spotify",
     pkg = {default = {"curl"}}},
    {bin = "playerctl", kind = "required",
     why = "transport control and the position clock",
     pkg = {default = {"playerctl"}}},
    -- EITHER daemon will do, and spoot drives whichever it finds -- so the
    -- second name is a real fallback rather than a preference: a distribution
    -- that packages librespot and not spotifyd gets a working install.
    {bin = "spotifyd", kind = "required", alt = "librespot",
     why = "playback itself",
     pkg = {default = {"spotifyd", "librespot"}},
     url = "https://spotifyd.rs"},
    -- Wayland first, X11 behind it, for the same reason.
    {bin = "wl-copy", kind = "required", alt = "xclip",
     why = "copying a web link",
     pkg = {default = {"wl-clipboard", "xclip", "xsel"}}},
    {bin = "notify-send", kind = "optional",
     why = "desktop notifications",
     pkg = {default = {"libnotify"}, ["apt-get"] = {"libnotify-bin"},
            zypper = {"libnotify-tools"}}},
    -- REQUIRED, because logging in is not optional: both OAuth flows below put a
    -- page in front of you, and this is what puts it there.
    {bin = "xdg-open", kind = "required",
     why = "opening the login page in your browser",
     pkg = {default = {"xdg-utils"}}},
    -- The OAuth callback listener is a perl one-liner (see oauth_get_token): it
    -- holds 127.0.0.1:8989 for the length of the login and writes the code it
    -- receives. Undeclared until now, which on a machine without perl would have
    -- made the login fail with nothing to point at.
    {bin = "perl", kind = "required",
     why = "receiving the login callback",
     pkg = {default = {"perl"}}},
    {bin = "openssl", kind = "required",
     why = "the login handshake",
     pkg = {default = {"openssl"}}},
    {bin = "pkill", kind = "required",
     why = "stopping spoot's own background helpers",
     pkg = {default = {"procps-ng", "procps"}}},
    {bin = "songrec", kind = "optional",
     why = "identifying what is playing in the room",
     pkg = {default = {"songrec"}},
     url = "https://github.com/marin-m/SongRec"}
}

-- EVERY MANAGER'S NO-CONFIRM FORM, which is the whole point of the exercise: a
-- prompt spoot cannot answer is the same as a failure, and worse, it hangs.
-- `root` says whether the command needs privilege at all -- nix and brew install
-- into the user's own profile and must NOT be run through sudo.
Util.PKG_MGRS = {
    {bin = "pacman",       root = true,
     cmd = "pacman -S --needed --noconfirm"},
    -- Updated first, deliberately. apt fails on a package it has never heard of,
    -- and an index that has not been refreshed since the image was built has
    -- heard of very little.
    {bin = "apt-get",      root = true,
     cmd = "apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y"},
    {bin = "dnf",          root = true,  cmd = "dnf install -y"},
    {bin = "zypper",       root = true,  cmd = "zypper --non-interactive install"},
    {bin = "apk",          root = true,  cmd = "apk add"},
    {bin = "xbps-install", root = true,  cmd = "xbps-install -Sy"},
    {bin = "emerge",       root = true,  cmd = "emerge --quiet --noreplace"},
    {bin = "pkg",          root = true,  cmd = "pkg install -y"},        -- FreeBSD
    {bin = "pkg_add",      root = true,  cmd = "pkg_add"},               -- Open/NetBSD
    {bin = "nix-env",      root = false, cmd = "nix-env -iA"},
    {bin = "brew",         root = false, cmd = "brew install"}
}

-- DOES THIS MANAGER CARRY THIS PACKAGE? One query per manager, phrased so that
-- an exit status alone answers it. Local index only wherever the manager has
-- one -- this runs at startup on a machine that is already missing things, and
-- a repository round trip is not something to spend a launch on.
--
-- A manager with no query here answers "yes" by default: attempting an install
-- and reporting what the manager said beats refusing to try.
Util.PKG_QUERY = {
    pacman         = "pacman -Si %s >/dev/null 2>&1",
    ["apt-get"]    = "apt-cache show %s >/dev/null 2>&1",
    dnf            = "dnf -q info %s >/dev/null 2>&1",
    zypper         = "zypper -q search --match-exact %s >/dev/null 2>&1",
    apk            = "apk search -e %s 2>/dev/null | grep -q .",
    ["xbps-install"] = "xbps-query -R %s >/dev/null 2>&1",
    pkg            = "pkg search -e %s >/dev/null 2>&1",
    pkg_add        = "pkg_info -Q %s 2>/dev/null | grep -q .",
    brew           = "brew info %s >/dev/null 2>&1"
}

function Util.pkg_exists(mgr, name)
    local q = Util.PKG_QUERY[mgr.bin]
    if not q then return true end
    -- The query is a program too, and it is not always the same program as the
    -- manager (xbps-install queries through xbps-query, pkg_add through
    -- pkg_info). If it is not there, we cannot ask -- and "cannot ask" must mean
    -- "assume yes", or a missing query tool would quietly demote every package
    -- to a manual instruction.
    local tool = q:match("^([%w_.-]+)")
    if tool and not Util.have(tool) then return true end
    Util._pkg_seen = Util._pkg_seen or {}
    local key = mgr.bin .. "/" .. name
    if Util._pkg_seen[key] ~= nil then return Util._pkg_seen[key] end
    local ok = os.execute(string.format(q, shell_quote(name))) and true or false
    Util._pkg_seen[key] = ok
    return ok
end
...
-- Linux, or one of the BSDs, or macOS. Only used to report -- what actually
-- decides everything below is which package manager is on PATH, which is the
-- more reliable question anyway: an Arch derivative that calls itself something
-- else still has pacman.
function Util.platform()
    if Util._platform then return Util._platform end
    Util._platform = trim(shell("uname -s 2>/dev/null") or "")
    if Util._platform == "" then Util._platform = "unknown" end
    return Util._platform
end

-- The distribution's own name for itself. os-release is the one file every
-- systemd-era Linux agrees on, and the BSDs simply do not have it.
function Util.distro()
    if Util._distro ~= nil then return Util._distro end
    local raw = read_file("/etc/os-release") or ""
    local id = raw:match('\nID=%s*"?([%w._-]+)"?') or raw:match('^ID=%s*"?([%w._-]+)"?')
    Util._distro = id or ""
    return Util._distro
end

-- Which manager a distribution's own name implies. A PREFERENCE, never a
-- requirement: it only reorders managers that are genuinely installed, so a box
-- with both nix and apt gets apt rather than whichever this file happens to list
-- first, and a distribution nobody here has heard of still works by falling
...
Util.DISTRO_MGR = {
    ...
    pop = "apt-get", raspbian = "apt-get", kali = "apt-get", devuan = "apt-get",
    ...
    alpine = "apk", void = "xbps-install", gentoo = "emerge", nixos = "nix-env",
    freebsd = "pkg", openbsd = "pkg_add", netbsd = "pkg_add"
}

function Util.pkg_mgr()
    if Util._mgr ~= nil then return Util._mgr or nil end
    local want = Util.DISTRO_MGR[Util.distro()]
    if want then
        for _, m in ipairs(Util.PKG_MGRS) do
            if m.bin == want and Util.have(m.bin) then Util._mgr = m; return m end
        end
    end
    for _, m in ipairs(Util.PKG_MGRS) do
        if Util.have(m.bin) then Util._mgr = m; return m end
    end
    Util._mgr = false
    return nil
end

-- HOW TO BECOME ROOT WITHOUT ASKING SPOOT TO HANDLE A PASSWORD, which it must
-- never do. In order of how little they cost the user:
--
--   root    already there, nothing to do
--   sudo -n / doas -n  a rule already grants it; fails instantly if not, and
--                      never prompts, so trying costs one process
--   pkexec  the desktop's own authentication dialog, which spoot does not draw,
--           does not read and cannot capture
--
-- If none of them fits, the answer is the command itself, printed. That is not a
-- failure mode -- for a lot of setups it is the correct one.
function Util.privilege()
    if trim(shell("id -u 2>/dev/null") or "") == "0" then
        return {method = "root", prefix = ""}
    end
    if Util.have("sudo") and os.execute("sudo -n true >/dev/null 2>&1") then
        return {method = "sudo", prefix = "sudo -n "}
    end
    if Util.have("doas") and os.execute("doas -n true >/dev/null 2>&1") then
        return {method = "doas", prefix = "doas -n "}
    end
    -- Not tested by running it: pkexec with nothing to do still raises a dialog,
    -- and asking the user to authenticate a probe is exactly the sort of thing
    -- that teaches people to click through prompts.
    if Util.have("pkexec") then return {method = "pkexec", prefix = "pkexec "} end
    return {method = false, prefix = ""}
end

-- What is missing, what can be installed, and the one command that would do it.
-- Answers the same shape whether or not anything is wrong, so callers never have
-- to special-case a healthy system.
function Util.dep_plan()
    local mgr = Util.pkg_mgr()
    local missing, packages, manual = {}, {}, {}
    for _, d in ipairs(Util.DEPS) do
        local has = Util.have(d.bin) or (d.alt and Util.have(d.alt))
        if not has then
            missing[#missing + 1] = {bin = d.bin, alt = d.alt, kind = d.kind,
                                     why = d.why, url = d.url}
            -- The first candidate this manager actually has. Nothing is
            -- assumed: a name that does not resolve is not put in the command,
            -- because one bad name fails the batch.
            local name = nil
            if mgr then
                local cands = d.pkg[mgr.bin] or d.pkg.default or {}
                for _, c in ipairs(cands) do
                    if Util.pkg_exists(mgr, c) then name = c; break end
                end
            end
            if name then packages[#packages + 1] = name
            else manual[#manual + 1] = {bin = d.bin, why = d.why, url = d.url} end
        end
    end
    local plan = {platform = Util.platform(), distro = Util.distro(),
                  manager = mgr and mgr.bin or "", missing = missing,
                  packages = packages, manual = manual}
    if #packages == 0 or not mgr then return plan end
    -- nix names its packages by attribute path, one prefix per package, so its
    -- argument list is built rather than concatenated.
    local args = {}
    for i, p in ipairs(packages) do
        args[i] = (mgr.bin == "nix-env") and ("nixpkgs." .. p) or p
    end
    local body = mgr.cmd .. " " .. table.concat(args, " ")
    if not mgr.root then
        plan.command, plan.method = body, "user"
        return plan
    end
    local priv = Util.privilege()
    plan.method = priv.method or "manual"
    -- The whole thing goes through one shell, because apt's form is two commands
    -- joined by && and only the shell knows what to do with that. /bin/sh by
    -- absolute path: pkexec runs with a scrubbed environment and will not go
    -- looking down a PATH for the program it has been asked to authenticate.
    plan.command = priv.method and (priv.prefix .. "/bin/sh -c " .. shell_quote(body)) or body
    -- What to show a user who has to run it themselves: the bare command with a
    -- plain sudo in front, since they CAN answer a prompt.
    plan.manual_command = "sudo " .. body
    return plan
end

-- The attempt, recorded. Not a lock -- a memory, so a machine that is missing
-- something unpackageable does not try again every single launch and print the
-- same failure. Lives in the cache because it describes this machine rather than
-- this session, and the cache is the one persistent place spoot owns.
function Util.dep_state()
    local raw = read_file(P.cache .. "/deps.json")
    if not raw then return {} end
    local ok, d = pcall(json.decode, raw)
    return (ok and type(d) == "table") and d or {}
end

function Util.dep_remember(plan, ok)
    local names = {}
    for i, m in ipairs(plan.missing) do names[i] = m.bin end
    table.sort(names)
    write_file(P.cache .. "/deps.json", json.encode({
        at = os.time(), missing = table.concat(names, ","), ok = ok and true or false
    }))
end

-- A day, and only for the SAME set of missing programs. Install something by
-- hand and the set changes, so the next launch looks again immediately rather
-- than sulking for the rest of the day.
function Util.dep_should_try(plan)
    if #plan.missing == 0 then return false end
    local st = Util.dep_state()
    local names = {}
    for i, m in ipairs(plan.missing) do names[i] = m.bin end
    table.sort(names)
    if st.missing ~= table.concat(names, ",") then return true end
    return (os.time() - (tonumber(st.at) or 0)) > 86400
end

-- Runs it. Output is captured rather than shown, because a package manager on a
-- terminal nobody is looking at is just noise -- what the user sees is the one
-- line saying whether it worked, and the log if it did not.
function Util.dep_install(plan)
    if not plan.command then return false, "nothing to install" end
    -- STDIN IS THE PROTOCOL. The engine reads requests from it, and a child
    -- process inherits it -- so a package manager that decides to ask a question
    -- would read the UI's next request as the answer and eat it off the socket.
    -- Nothing here is meant to prompt; this is what makes that true rather than
    -- hoped for, and turns a hung install into a failed one.
    --
    -- And a ceiling, because unattended means nobody is watching it: a manager
    -- waiting on a lock held by a system update would otherwise hold the engine
    -- for as long as that took. Only when `timeout` is actually present, which
    -- it is on Linux and the BSDs alike but is not worth assuming.
    local guard = Util.have("timeout") and "timeout 600 " or ""
    local out = shell("{ " .. guard .. plan.command .. " ; } </dev/null 2>&1") or ""
    -- Believe the SYSTEM, not the exit code. Package managers disagree wildly
    -- about what a partial success is worth, and the only thing that actually
    -- matters is whether the programs are there now.
    Util._mgr, Util._platform, Util._distro = nil, nil, nil
    local after = Util.dep_plan()
    local ok = #after.missing < #plan.missing
    Util.dep_remember(after, ok)
    return ok, out, after
end

-- The report, as text. Shared by --doctor and the crash-free path through the
-- UI, so the two cannot describe the same machine differently.
function Util.dep_report(plan)
    local L = {}
    L[#L+1] = "platform: " .. plan.platform
        .. (plan.distro ~= "" and ("  distro: " .. plan.distro) or "")
        .. (plan.manager ~= "" and ("  package manager: " .. plan.manager)
                                or "  package manager: none found")
    if #plan.missing == 0 then
        L[#L+1] = "all dependencies present"
        return table.concat(L, "\n")
    end
    for _, m in ipairs(plan.missing) do
        L[#L+1] = string.format("  missing  %-13s %s%s", m.bin,
            m.kind == "optional" and "(optional) " or "", m.why)
    end
    if plan.command then
        L[#L+1] = ""
        L[#L+1] = "install: " .. (plan.manual_command or plan.command)
    end
    for _, m in ipairs(plan.manual) do
        L[#L+1] = "  by hand: " .. m.bin .. (m.url and ("  " .. m.url) or "")
    end
    return table.concat(L, "\n")
end
...
function Util.setup_notify(title, body)
    if not Util.have("notify-send") then return end
    os.execute("notify-send --app-name=spoot " .. shell_quote(title)
        .. " " .. shell_quote(body or "") .. " 2>/dev/null")
end
...
function Util.device_ready()
    ...
    for _, dir in ipairs({"oauth", "zeroconf"}) do
        if read_file(P.spotifyd .. "/" .. dir .. "/credentials.json") then return true end
    end
    -- Older spotifyd (pre-0.4) used <cache>/credentials.json directly.
    return read_file(P.spotifyd .. "/credentials.json") ~= nil
end
...
-- first and are not negotiable: the account login needs curl to exchange its
-- code, openssl to build the challenge, perl to catch the callback and xdg-open
-- to show you the page. Authorising before those exist fails four ways.
--
...
function Util.setup_state()
    local names = {"curl", "openssl", "perl", "xdg-open"}
    local lack = {}
...
local function ensure_daemon()
    local daemon_alive = Util.pidfile_owner_alive(P.daemon_pid, "--daemon")
...
function Util.ensure_recent_watch()
    local alive = Util.pidfile_owner_alive(P.recent_pid, "--recent-watch")
    if not alive then
...
function Util.kill_recent_watch()
    local wp = trim(read_file(P.recent_pid) or "")
...
local function check_rate_cooldown()
    local rate_cool = read_file(P.rate_cooldown)
    if rate_cool then
...
local function main()
    ...
    while true do
        ...
        -- playing track, folded in by hand because rofi_dmenu only prepends a
        ...
        local sel = rofi_dmenu(entries, {prompt="Spotify", mesg=mesg, by_index=true, thumbs=true,
...
local function daemon_mode()
    ...
    local NOTIFY_FILE = P.last_notify
...
function Util.recent_watch_mode()
    ...
    while true do
        local cooldown = tonumber((read_file(P.rate_cooldown) or ""):match("%d+"))
...
-- SERVE MODE -- the headless engine behind spoot-next's Qt Quick front end.
--
...
Util.SERVE_VIEWS = {
    ...
    -- handled inside rofi_dmenu (EXIT.lyrics, EXIT.seek, EXIT.jump); replacing
...
--
-- The interception is the whole trick: with rofi_dmenu replaced by a recorder
...
function Util.serve_draw(name, d, raw)
    ...
    return {
        ...
        -- already names one when it calls rofi_dmenu, and those files carry the
...
function Util.serve_ctx_art()
    ...
    --
    -- A track list wears the playing track's art beside it, and until now that
    -- only happened as a SIDE EFFECT of having just played something: the step
    -- you took named the item, and the item named the cover. Walk back to Liked
    -- an hour later and no step had been taken, so the list came up bare -- and
    -- when autoplay moved on, the cover stayed on the track you had started
    -- rather than the one actually playing.
    --
    -- Resolved through the same album_thumbs path as any other cover, in the
    -- continuation, so it costs the draw nothing.
    local item = Util.serve_ctx_item
...
function Util.serve_art_after(name, d)
    ...
    Util.serve_after = function()
        ...
        -- EVERY ROW'S OWN COVER, for a list. The backdrop follows the cursor as
        -- you move it, and it can only do that if the UI already holds a path
        -- for each row -- asking per keypress would be a request per keypress.
        --
        -- Cache-only, so this is a hash and a stat per row and never a download:
        -- a row whose cover is not on disk simply has none, and the backdrop
        -- falls back to the playing track's. Lists only -- a grid's tiles carry
        -- their own artwork already, and overwriting those with a different
        -- resolution would make the grid reload for nothing.
        local it = d and d.opts and d.opts.items
        if type(it) == "table" and not (d.opts and d.opts.thumbs)
           and #it == #(d.entries or {}) then
            local covers = {}
            for i, t in ipairs(it) do
                local u = type(t) == "table" and t.album and t.album.images
                          and t.album.images[1] and t.album.images[1].url or nil
                if u then
                    local pth = Util.ensure_art_med(u, true)
                    if pth ~= "" then covers[#covers + 1] = {i = i, icon = pth} end
                end
            end
            if #covers > 0 then
                Util.serve_write({ev = "art", view = name, items = covers})
            end
        end
        ...
        Util.serve_write({ev = "context-art", view = name, path = Util.serve_ctx_art() or ""})
    end
end
...
function Util.serve_control(args)
    ...
    elseif a == "playpause" then os.execute("playerctl -p spotifyd play-pause 2>/dev/null")
    elseif a == "next" then os.execute("playerctl -p spotifyd next 2>/dev/null")
    elseif a == "prev" then os.execute("playerctl -p spotifyd previous 2>/dev/null")
    elseif a == "seek" then
        -- Seconds, signed. playerctl takes "10+" / "10-" rather than a sign.
        local by = tonumber(args.by) or 10
        os.execute("playerctl -p spotifyd position " .. math.abs(by)
            .. (by < 0 and "-" or "+") .. " 2>/dev/null")
...
-- The dependency scan, and the install it plans. Reporting is free; installing
-- happens only when asked for, so nothing here can escalate on its own.
Util.serve_deps = function(args)
    local plan = Util.dep_plan()
    if args and args.install and plan.command then
        local ok, out, after = Util.dep_install(plan)
        return {ok = ok, report = Util.dep_report(after),
                missing = Util.dep_names(after.missing),
                -- Enough of the tail to see what a package manager objected to,
                -- and not so much that a full apt transcript rides the socket.
                log = (out or ""):sub(-1500)}
    end
    return Util.dep_summary(plan)
end

-- One shape for the scan, wherever it is asked for -- the startup event and the
-- command answer the same thing, so the UI has one thing to read.
function Util.dep_names(list)
    local out = {}
    for i, m in ipairs(list or {}) do out[i] = m.bin end
    return out
end

function Util.dep_summary(plan)
    local required = 0
    for _, m in ipairs(plan.missing) do
        if m.kind ~= "optional" then required = required + 1 end
    end
    return {missing = Util.dep_names(plan.missing), required = required,
            manager = plan.manager, platform = plan.platform,
            distro = plan.distro, method = plan.method or "",
            command = plan.manual_command or plan.command or "",
            report = Util.dep_report(plan),
            -- Whether the UI should go ahead on its own. A plan with no command
            -- has nothing to run, and a set already tried today is not tried
            -- again -- see dep_should_try.
            --
            -- And an authentication dialog is only ever raised for something
            -- spoot cannot work without. Optional extras ride along inside that
            -- same command when it happens -- so nothing is left behind -- but
            -- they never summon the prompt themselves: being asked for a
            -- password to install a feature you have not used is how people
            -- learn to click through prompts without reading them.
            auto = (plan.command ~= nil) and Util.dep_should_try(plan)
                   and (required > 0 or plan.method == "root"
                        or plan.method == "sudo" or plan.method == "doas"
                        or plan.method == "user") or false}
end
...
Util.SERVE = {
    ...
    -- Copies the playing track's web link, the way Alt+g does. Answers the URL
    -- so the UI can say what it put on the clipboard rather than guessing.
    weblink = function()
        if not Util.fast_now_track() then last_playback = 0; get_playback() end
        if not current_track or not current_track.id then error("nothing playing", 0) end
        copy_spotify_url("track", current_track.id)
        return {url = Util.web_url("track", current_track.id), name = current_track.name}
    end,
...
function Util.serve_mode()
    Util.serving = true       -- read by views that spawn rofi themselves
    Util.detached = true          -- no rofi_message can reach a UI that is not rofi
    ensure_cache()
    ...
    session_load()
    Util.trail_load()
    Util.serve_saved = json.decode(json.encode(_session_stack or {}))
    -- THE INTERCEPTION. rofi_dmenu is a file local, so assigning it here rebinds
    ...
    rofi_dmenu = function(entries, opts)
        opts = opts or {}
        if Util.serve_capture then
            ...
            -- A plain name since themes stopped being files. The match is kept
            -- only so a path, if one ever arrives again, still reduces to its
            -- name rather than being reported verbatim.
            local th = opts.theme or (opts.thumbs and Util.THEME_THUMBS or THEME_MENU)
            th = th and (tostring(th):match("([^/]+)%.rasi$") or tostring(th)) or nil
            ...
            -- counting raw rofi_dmenu calls handed those redraws the answer
            ...
            -- The SEARCH HISTORY list is the one menu whose Delete rofi_dmenu
            ...
            -- rofi_dmenu used to handle Shift+Return ITSELF for lists that do
            -- not claim it (EXIT.alt_action): a track row has no alt branch in
            -- view_browse because rofi opened the action menu on the list's
            -- behalf. Replacing rofi_dmenu took that with it, so the default
            ...
            -- expression rofi_dmenu uses -- the default is computed there, not
            ...
            -- -- hands rofi_dmenu a `refresh` and lets it rebuild them; rofi
    ...
    -- Rename Playlist and New Playlist ask for TEXT, and rofi_input spawns rofi
    -- on its own rather than going through rofi_dmenu -- so without this the one
    ...
    rofi_input = function(prompt, preset, _theme)
    ...
    rofi_message = function(msg, theme)
        -- The theme is the sheet's GEOMETRY: meta is 800px, binds 680, pods
        -- 1000, and the default message 700. Normalised exactly as the menu
        -- themes are, so both paths name a theme the same way and the UI has one
        -- table to look either up in.
        local th = theme and (tostring(theme):match("([^/]+)%.rasi$") or tostring(theme)) or nil
    ...
    if not os.getenv("SPOOT_NO_AUTOSETUP") then
        local ok, plan = pcall(Util.dep_plan)
        if ok and plan and #plan.missing > 0 then
            local ev = Util.dep_summary(plan)
            ev.ev = "deps"
            Util.serve_write(ev)
        end
        ...
        if line ~= "" then
            ...
            if not req or not req.cmd then
                ...
                if not fn then
                    ...
                    else Util.serve_write({id = req.id, ok = false, err = tostring(data)}) end
...
if arg and arg[1] == "--doctor" then
    local plan = Util.dep_plan()
    print(Util.dep_report(plan))
    print("")
    print(Util.setup_report())
    if arg[2] == "install" and plan.command then
        if #plan.packages == 0 then os.exit(0) end
        print("\n$ " .. (plan.manual_command or plan.command))
        -- The interactive form deliberately: a person is watching, so a password
        -- prompt is a question rather than a hang.
        local cmd = plan.method == "user" and plan.command
                    or (plan.manual_command or plan.command)
        os.execute(cmd)
        local after = Util.dep_plan()
        Util.dep_remember(after, #after.missing == 0)
        print("")
        print(Util.dep_report(after))
        os.exit(#after.missing == 0 and 0 or 1)
    end
    os.exit(#plan.missing == 0 and 0 or 1)
