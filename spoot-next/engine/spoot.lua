...
function Util.search_page_pick(results, page)
    ...
    local idx = rofi_dmenu(rows, {prompt="Type", mesg="Filter results by type", by_index=true,
                                  theme=THEME_SUB, art=false})
    ...
    while true do
        ...
        if n and n ~= cur then
            ...
            local c = rofi_dmenu({"Restart", "Abort"},
                {prompt="Bitrate", mesg=n .. " kbps" .. SEP .. "restart daemons to apply?", by_index=true, theme=THEME_SUB, confirm=true, art=false})
            if c == 1 then
        ...
        if sel == 3 then
            if not sz then
                ...
                local c = rofi_dmenu({"Clear Cache", "Abort"},
                    {prompt="Track Cache", mesg="delete " .. sz .. " of cached tracks?", by_index=true, theme=THEME_SUB, confirm=true, art=false})
                if c == 1 then
...
Util.SERVE_VIEWS = {
    ...
    ["open-link"]    = function()
        local url = Util.get_clipboard()
        if url == "" then rofi_message("Clipboard is empty"); return end
        open_url(url)
    end,
...
function Util.serve_ctx_art(d)
    ...
    -- album_thumbs answers with the 300px grid tile, and this is drawn at 364
    -- and up. It is the same picture at the tier that fits.
...
function Util.serve_mode()
    ...
    rofi_dmenu = function(entries, opts)
        ...
        if Util.serve_capture then
            ...
            -- same rofi_dmenu call with the same opts: its prompt cannot differ.
            ...
            if type(ans) == "number" and type(opts.items) == "table"
               and type(entries) == "table" and #opts.items == #entries then
                ...
                -- that has one records it on the way in (Util.write_art_theme),
