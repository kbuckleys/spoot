...
Item {
    ...
    // THE COPY MARK. Copying a link is over the instant it happens, and the only
    // thing worth saying is that it happened -- so it is said beside the track it
    // copied and then withdrawn, rather than as a sentence you have to read and
    // then wait out.
    //
    // Its own item, not folded into the title above: it fades on its own, and a
    // centred line that grew a glyph would shift every word in it sideways as the
    // mark came and went.
    Text {
        anchors { left: title.right; leftMargin: 8
                  verticalCenter: parent.verticalCenter }
        text: content.theme.glyphCopied
        color: content.fg
        font.family: content.theme.fontFamily
        font.pointSize: content.theme.fontSize - 1
        font.bold: true
        opacity: content.copied ? 1 : 0
        visible: opacity > 0
