...
Window {
    ...
    function bootstrap() {
        // `spoot --listen` goes straight there; the host sets startView.
        if (typeof startView !== "undefined" && startView === "listen") {
            root.openView("listen")
            return
        }
    ...
    //
    // Two places it can land, because there are two ways to ask for it. Picking
    // "Copy Web Link" out of an action menu marks THAT ROW, at its right edge;
    // Alt+g from anywhere has no row to mark, so it marks the track it copied,
    // in the now-playing strip.
    property bool copiedFlash: false          // Alt+g -- see NowContent
    property int lastSrc: -1                  // the row the last step picked
    property int copiedSrc: -1                // ...and the one that copied, if any
    Timer {
        id: copiedTimer
        interval: 1000
        onTriggered: { root.copiedFlash = false; root.copiedSrc = -1 }
    }
    Timer {
        id: noticeTimer
    ...
    function applyArt(data) {
        ...
        for (var k = 0; k < items.length; k++) {
            ...
            if (rows.get(j).icon !== icon) { rows.setProperty(j, "icon", icon); root.artRev++ }
        }
    }
    ...
    function patchRows(list) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            var icon = r.icon || ""
            if (m.icon !== icon) rows.setProperty(i, "icon", icon)
            var key = r.key || ""
    ...
    // ...and in a track list, the cover follows the CURSOR: moving to a track
    // from a different album fades that album in. The row carries its own path
    // (see the art continuation), so this costs no request and no redraw -- the
    // two stacked images cross-fade and nothing else on screen moves.
    //
    // Falls through where a row has no cover of its own: an action menu's verbs,
    // a settings list, a track whose art is not on disk yet.
    readonly property int selIndex: body.item ? body.item.currentIndex : -1
    // Bumped whenever a cover lands. ListModel.get() is a snapshot, so a binding
    // that reads it re-runs when the INDEX changes and never when the row does --
    // which meant the backdrop was whatever had been resolved at the moment the
    // cursor last moved, and covers arriving a beat later were simply not seen.
    // That is the mismatch: the cover of a row you had already left.
    property int artRev: 0
    readonly property string selArt: {
        if (root.artRev < 0) return ""          // read, so this depends on it
        if (root.layout !== "list") return ""
        if (root.selIndex < 0 || root.selIndex >= rows.count) return ""
        return rows.get(root.selIndex).icon || ""
    }
    readonly property string coverArt:
        root.selArt.length ? root.selArt
    ...
    function closeOverlay() {
        ...
        var wasArt = root.artPath.length > 0
        if (wasArt) {
    ...
    function activateCurrent(alt) {
        if (rows.count && body.item) root.activate(body.item.currentIndex, alt === true)
    }
    ...
    function copyWebLink() {
        root.call("weblink", {}, function (d) {
            // Success says so with a mark beside the track, not a sentence.
            // Failure still needs words -- "nothing happened" is the one thing a
            // glyph on the track you do not have cannot express.
            if (d && d.url) { root.copiedFlash = true; copiedTimer.restart() }
            else root.notify("Nothing playing")
        })
    }
    ...
    Item {
        ...
        anchors.verticalCenter: root.artPath.length ? parent.verticalCenter : undefined
        anchors.bottom: root.artPath.length ? undefined : parent.bottom
