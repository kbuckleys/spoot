...
//
// What it does:
//   1. runs the Lua engine on a worker thread and passes newline-delimited JSON
//      both ways across that boundary, the same lines stdio would carry,
//   2. exposes that channel to QML as one object with one method and two signals,
//   3. anchors the window to the screen through wlr-layer-shell,
//   4. loads the UI from disk at runtime,
//   5. gives the engine the natives it would otherwise fork for -- HTTP, MPRIS,
//      notifications, the clipboard, the OAuth callback -- plus the few things
//      only a process-wide owner can do safely: named locks, a shared number
//      store and a pool of background job states.
//
// Everything else -- every view, every pixel, every keybinding -- lives in
// ui/*.qml or engine/*.lua and needs no rebuild to change.
...
// ONE NETWORK MANAGER PER THREAD, living as long as the thread does. A
// QNetworkAccessManager is where kept-alive connections live, so one per JOB --
// which is what a Natives member was -- threw away every TLS session the moment
// the job ended, and the next job paid a cold TCP+TLS handshake to the same host
// all over again (~0.6s against ~0.1s warm, measured on the API). The pool's
// threads now outlive their jobs, and so does this. QThreadStorage deletes it on
// its own thread when that thread exits.
static QNetworkAccessManager *threadNam() {
    static QThreadStorage<QNetworkAccessManager *> nams;
    if (!nams.hasLocalData()) nams.setLocalData(new QNetworkAccessManager());
    return nams.localData();
}
...
public:
    ...
    void install(lua_State *L, const char *role, const QByteArray &busName) {
        ...
        lua_pushcclosure(L, &Natives::l_with_lock, 0);
...
private:
    ...
    // because nothing on this thread calls back into Lua: the only events the
    // nested loop can see are this reply's and those of earlier `bg` replies
    // finishing, none of which touch the Lua state. The GUI thread is untouched
    // throughout and keeps drawing.
    static int l_http(lua_State *L) {
        ...
        // for a frame. Nothing waits; the reply cleans itself up. It is SENT
        // while the worker idles in l_next, which dispatches this thread's
        // events -- without that it would wait for the next nested loop.
        lua_getfield(L, 1, "bg");
    ...
    static int l_http_batch(lua_State *L) {
        ...
        lua_pop(L, 1);
        // IN MEMORY, WHERE THE CALLER ONLY WANTS TO READ IT. A page of JSON
        // written to a temp file here was read straight back, decoded and
        // deleted by the engine -- a write, a read and an unlink per page for
        // bytes that were already in this process. `bodies` returns them in the
        // result instead, and `out` is then only the name the result is filed
        // under. Artwork still wants files, so it simply does not ask.
        lua_getfield(L, 1, "bodies");
        const bool bodies = lua_toboolean(L, -1);
        lua_pop(L, 1);
        ...
        for (int i = 0; i < reps.size(); ++i) {
            ...
            if (bodies) {
                wrote = body.size();
            } else if (code >= 200 && code < 300 && !outs[i].isEmpty()) {
                QFile f(outs[i]);
            ...
            if (bodies) {
                lua_pushlstring(L, body.constData(), size_t(body.size()));
    ...
    // else.
    //
    // Player resolution: the org.mpris.MediaPlayer2.* name whose suffix contains
    // `player`, the way playerctl's -p matches. The choice is remembered and
    // dropped the moment a call to it fails, which is what happens when
    // spotifyd restarts.
    //
    // NO HINT MEANS SPOTIFYD, NOT "THE FIRST PLAYER". Twenty of the engine's
    // twenty-four calls name no player, and this used to answer them with the
    // first name on the bus -- a browser, mpv, playerctld, in whatever order the
    // bus lists them. So the action menu's Pause could pause a YouTube tab, and a
    // track change there could come up as spoot's now-playing and toast. spoot
    // drives one daemon (see kPlayer); a player that is not it is nobody's.
    QString mprisName(const QByteArray &want) {
        const QString hint = want.isEmpty() ? QString(kPlayer) : QString::fromUtf8(want);
        if (!m_player.isEmpty() && m_player.mid(23).contains(hint)) return m_player;
        ...
        for (const QString &n : names.value()) {
            ...
            if (n.mid(23).contains(hint)) { m_player = n; return n; }
        }
        return QString();
    }
    ...
    // ── ONE AT A TIME, ACROSS STATES ─────────────────────────────────────────
    // spoot.with_lock(name, fn): runs fn with a process-wide mutex of that name
    // held, and answers whatever fn answers. The engine and every job are Lua
    // states on their own threads, sharing nothing in memory -- so anything they
    // must not do concurrently, like spending one refresh token twice (see
    // get_token), has to be serialised here, below all of them.
    //
    // Recursive, so a state that re-enters the same name cannot deadlock itself.
    // Unlocked by hand, never by a guard: Lua errors unwind with longjmp, which
    // skips C++ destructors, so the pcall catches fn's error, the lock is
    // released, and only then is the error raised again.
    static std::recursive_mutex &namedLock(const QByteArray &name) {
        static std::mutex registry;
        static std::map<QByteArray, std::unique_ptr<std::recursive_mutex>> locks;
        std::lock_guard<std::mutex> lk(registry);
        auto &m = locks[name];
        if (!m) m = std::make_unique<std::recursive_mutex>();
        return *m;
    }

    static int l_with_lock(lua_State *L) {
        size_t n = 0;
        const char *name = luaL_checklstring(L, 1, &n);
        luaL_checktype(L, 2, LUA_TFUNCTION);
        lua_settop(L, 2);
        luaL_checkstack(L, 1, "with_lock");   // before locking: it can raise
        std::recursive_mutex &mx = namedLock(QByteArray(name, int(n)));
        mx.lock();
        lua_pushvalue(L, 2);
    ...
    // spoot.shared(name) / spoot.shared(name, number|nil): one number per name,
    // visible to every Lua state in the process. What the rate-limit gate lives
    // in: it is read before EVERY request, by the engine and by every job, and
    // it used to be a file -- a read per request, and a write plus a chmod fork
    // per 429. In memory it is a lookup, and the states still agree on it.
    static std::mutex &sharedLock() { static std::mutex m; return m; }
    static QHash<QByteArray, double> &sharedStore() { static QHash<QByteArray, double> s; return s; }

    static int l_shared(lua_State *L) {
        size_t n = 0;
        const char *k = luaL_checklstring(L, 1, &n);
        const QByteArray key(k, int(n));
        if (lua_gettop(L) >= 2) {
            const bool clear = lua_isnil(L, 2);
            const double v = clear ? 0 : luaL_checknumber(L, 2);
            std::lock_guard<std::mutex> lk(sharedLock());
            if (clear) sharedStore().remove(key); else sharedStore().insert(key, v);
            return 0;
        }
        std::lock_guard<std::mutex> lk(sharedLock());
        auto it = sharedStore().constFind(key);
        if (it == sharedStore().constEnd()) lua_pushnil(L); else lua_pushnumber(L, it.value());
        return 1;
    }
    ...
    // Answers the callback's QUERY STRING -- code or error, and the state it
    // came back with -- or nil on timeout, and the engine reads it apart. The
    // server is closed either way: a failed login must not leave the port held
    // for the next attempt.
    //
    // THE STATE IS CHECKED HERE when one is given, so a request carrying some
    // other login's answer (or a local process guessing at the port) is turned
    // away with the socket still open for the real one. An error= answer ends
    // the wait at once: it is the user pressing Cancel on Spotify's consent
    // page, and nothing else is coming after it.
    static int l_oauth_wait(lua_State *L) {
        Natives *w = self(L);
        Q_ASSERT(QThread::currentThread() == w->m_owner);
        const int secs = int(luaL_optinteger(L, 1, 120));
        const QString want = QString::fromUtf8(luaL_optstring(L, 2, ""));
        if (!w->m_oauth) { lua_pushnil(L); return 1; }
        QDeadlineTimer deadline(qint64(secs) * 1000);
        QString query;
        while (query.isEmpty() && !deadline.hasExpired()) {
            ...
            // A request that is NOT the callback is not a failure. A browser
            // asks for /favicon.ico beside the page it was sent to, and treating
            // the first connection as the answer would have made that race the
            // login.
            static const QRegularExpression line(QStringLiteral("^GET /login\\?(\\S+)"));
            static const QRegularExpression answer(QStringLiteral("(^|&)(code|error)="));
            const QRegularExpressionMatch m = line.match(QString::fromUtf8(req));
            QString q = m.hasMatch() ? m.captured(1) : QString();
            if (!answer.match(q).hasMatch()) q.clear();
            if (!q.isEmpty() && !want.isEmpty()
                && QUrlQuery(q).queryItemValue(QStringLiteral("state")) != want) q.clear();
            const bool cancelled = !q.isEmpty() && QUrlQuery(q).hasQueryItem(QStringLiteral("error"));
            ...
            const QByteArray page = q.isEmpty()
                ? QByteArrayLiteral("Not a login spoot is waiting for.")
                : cancelled ? QByteArrayLiteral("spoot login cancelled. You can close this.")
                            : QByteArrayLiteral("spoot has your login. You can close this.");
            c->write(QByteArray(q.isEmpty() ? "HTTP/1.1 400 Bad Request\r\n" : "HTTP/1.1 200 OK\r\n")
        ...
        delete w->m_oauth;
        w->m_oauth = nullptr;
        if (query.isEmpty()) { lua_pushnil(L); return 1; }
    ...
    static int l_mpris(lua_State *L) {
        ...
        // three Get calls. GetAll answers all three at once. Position is absent
        // from the answer on a player that does not export it through GetAll, in
        // which case the field is simply left out and the caller asks for it on
        // its own.
        if (op == "state") {
            QDBusMessage call = QDBusMessage::createMethodCall(
                name, QStringLiteral("/org/mpris/MediaPlayer2"),
            ...
            QVariantMap all;
            m.arguments().at(0).value<QDBusArgument>() >> all;
            lua_newtable(L);
            lua_newtable(L);
            const QByteArray st = all.value(QStringLiteral("PlaybackStatus")).toString().toUtf8();
            lua_pushlstring(L, st.constData(), size_t(st.size()));
            lua_setfield(L, -2, "status");
            if (all.contains(QStringLiteral("Position"))) {
                lua_pushnumber(L, double(all.value(QStringLiteral("Position")).toLongLong()) / 1e6);
                lua_setfield(L, -2, "position");
            }
            if (all.contains(QStringLiteral("Volume"))) {
                lua_pushnumber(L, all.value(QStringLiteral("Volume")).toDouble());
    ...
    QNetworkAccessManager *m_nam = nullptr;   // borrowed: see threadNam
...
public:
    JobRunner(QString script, QStringList args, JobPool *pool, QByteArray busName,
        ...
        : m_script(std::move(script)), m_args(std::move(args)), m_pool(pool),
          m_busName(std::move(busName)), m_bytecode(std::move(bytecode)) {}
...
public slots:
    void run() {
        ...
        // THE PRE-COMPILED ENGINE when the pool has one (see JobPool::bytecode):
        // ~0.5ms to load against ~10ms to parse 17,000 lines from source, per job.
        // Same chunk, same `@path` name, so tracebacks read exactly as before.
        // Straight from the file when there is none -- which is also what reports
        // a syntax error, since that is the one reason compiling fails.
        int rc;
        if (!m_bytecode.isEmpty()) {
            const QByteArray chunk = "@" + script;
            rc = luaL_loadbufferx(L, m_bytecode.constData(), size_t(m_bytecode.size()),
                                  chunk.constData(), "b");
            if (rc == LUA_OK) rc = lua_pcall(L, 0, LUA_MULTRET, 0);
        } else {
            rc = luaL_dofile(L, script.constData());
        }
        if (rc != LUA_OK) {
...
public:
    ...
    // Stop taking work, and wait up to `ms` for the jobs in flight. A job
    // cannot be interrupted -- it is a Lua state inside a blocking call -- so a
    // false here means one is still running and the process has to leave
    // without destroying it. See main().
    bool settle(int ms) {
        {
        ...
        QDeadlineTimer until(ms);
        for (QThread *t : std::as_const(m_threads)) {
            t->quit();
            if (!t->wait(until)) return false;
        }
        return true;
    }
...
signals:
    // A KEYED JOB LANDED. The engine is told (see Engine) because some of what a
    // job writes is something a menu already on screen is waiting to know -- a
    // lyrics lookup deciding whether an open action card keeps its Lyrics row.
    void jobDone(const QString &key);

private:
    // THE ENGINE, COMPILED ONCE. Every job is a fresh Lua state running the whole
    // script, and parsing 17,000 lines of it was ~10ms of every job's start --
    // one per track change, per revalidation, per cover spool. Compiled here, on
    // this thread, and handed to each runner as bytes to load.
    //
    // Recompiled when the file changes (mtime or size), so an edit to the engine
    // still reaches the next job without a restart, exactly as reading the file
    // every time did. Empty when it will not compile; the runner then reads the
    // file itself and reports the syntax error the way it always has.
    QByteArray bytecode() {
        const QFileInfo fi(m_script);
        const QDateTime stamp = fi.lastModified();
        if (!m_bc.isEmpty() && stamp == m_bcStamp && fi.size() == m_bcSize) return m_bc;
        QByteArray out;
        lua_State *L = luaL_newstate();
        const QByteArray path = m_script.toUtf8();
        if (luaL_loadfile(L, path.constData()) == LUA_OK) {
            // Not stripped: line numbers stay, so a job's traceback names the
            // same lines the source would.
            lua_dump(L, [](lua_State *, const void *p, size_t sz, void *ud) -> int {
                if (p && sz) static_cast<QByteArray *>(ud)->append(static_cast<const char *>(p),
                                                                   qsizetype(sz));
                return 0;
            }, &out, 0);
        }
    ...
    void pump() {
        for (;;) {
            ...
            // A THREAD THAT OUTLIVES ITS JOB. Up to MAX of them, started on
            // demand and then kept: each holds its network manager's warm
            // connections (see threadNam) for the next job to reuse. The Lua
            // state is still new every time -- that isolation is the point of a
            // job -- only the thread and its sockets carry over.
            QThread *thread = m_idle.isEmpty() ? nullptr : m_idle.takeLast();
            if (!thread) {
            ...
            connect(runner, &JobRunner::finished, this, [this, thread, runner, key = job.key] {
                ...
                // Deleted on its own thread, by that thread's loop, before the
                // next job it is handed can start.
                runner->deleteLater();
                m_idle.append(thread);
                // Said AFTER the key is released, so whoever hears it and asks
                // for the same job again is not refused as a duplicate.
                if (!key.isEmpty()) emit jobDone(key);
                pump();
            });
            QMetaObject::invokeMethod(runner, &JobRunner::run, Qt::QueuedConnection);
        }
    }
    ...
    QSet<QString> m_keys;
    QQueue<Job> m_queue;
    QSet<QThread *> m_threads;    // every pool thread, busy or not
    QList<QThread *> m_idle;      // the ones waiting for a job
    int m_running = 0;
    quint64 m_seq = 0;
...
// slot, which would need exactly that loop. While it waits for the next one it
// DOES dispatch its own thread's events (see l_next), so work it started, like a
// fire-and-forget request, keeps moving between commands. Replies go the other
// way as a queued signal, which Qt marshals onto the GUI thread for us.
...
private:
    ...
    //
    // THE WAIT DISPATCHES THIS THREAD'S EVENTS. It was a condition variable, and
    // a thread asleep on one hears nothing: a `bg` request (a shuffle or repeat
    // toggle) is only STARTED by a queued event on this thread, so it sat unsent
    // until some later request happened to spin a nested loop -- which, with
    // music playing and every poll answered from MPRIS, could be minutes.
    // Measured in a harness: the PUT reached the server 3s late, exactly when
    // the next loop ran. Now idle time is spent in the dispatcher, post() wakes
    // it, and a fire-and-forget request leaves the moment it is issued.
    static int l_next(lua_State *L) {
        LuaWorker *w = self(L);
        for (;;) {
            {
                std::lock_guard<std::mutex> lk(w->m_mx);
                // STOP WINS OVER THE QUEUE. It was checked second, so a worker
                // told to stop went on answering whatever was still queued --
                // and at Quit there is always something: killing spotifyd sets
                // the MPRIS watchers off and the playback poll keeps ticking.
                // The host's wait ran out on it, destroyed a running thread, and
                // Qt aborts on that: the crash the crash guard then "recovered"
                // by launching spoot again.
                if (w->m_stop) { lua_pushnil(L); return 1; }
                if (!w->m_in.isEmpty()) {
            ...
            // A wake-up that lands between the check above and this call is not
            // lost: it leaves the dispatcher primed, so this returns at once.
            QThread::currentThread()->eventDispatcher()
                ->processEvents(QEventLoop::WaitForMoreEvents);
        }
    }

    // Thread-safe by Qt's contract. Null only before the thread has started,
    // and then the request is simply found on the queue by the first check.
    void wake() {
        if (QAbstractEventDispatcher *d = thread()->eventDispatcher()) d->wakeUp();
    }
...
public:
    explicit Engine(const QString &script, QObject *parent = nullptr)
        ...
        // Every keyed job's end, posted in as a request like the watchers' --
        // the engine decides what, if anything, it means (Util.SERVE's job-done).
        connect(&m_pool, &JobPool::jobDone, this, [this](const QString &key) {
            request(QStringLiteral("job-done"), QVariantMap{{QStringLiteral("key"), key}});
        });
        spawn();
    }
...
private:
    ...
    void shutdown() {
        if (!m_worker) return;
        m_worker->stop();
        if (m_thread && !m_thread->wait(3000)) m_stuck = true;
        m_worker = nullptr;
        m_thread = nullptr;
    }

public:
    // EVERYTHING THAT RUNS ON ITS OWN THREAD, WOUND DOWN, before main returns.
    // True when it all stopped; false when something is still running -- a job
    // inside a slow request, say -- and then the caller must not let ordinary
    // destruction reach it: destroying a running QThread is fatal in Qt.
    bool settle(int ms) {
        m_stopping = true;
        shutdown();
        return m_pool.settle(ms) && !m_stuck;
    }
private:
...
//
// BUTTONS NEED NO ID BOOKKEEPING. Every action key spoot sends is namespaced
// `spoot:`, so the prefix is what says a button press is ours, and a toast that
// outlives a restart still works. The BODY is the exception: its key is always
// `default`, whoever sent the toast, so a click on it is matched against the last
// few ids spoot raised -- see rememberToast.
...
// A DELIBERATE QUIT IS NOT A CRASH TO RECOVER FROM. Anything that faults while
// spoot is shutting down -- a teardown order Qt objects to, a thread still inside
// a request -- used to reach the handler, which execs spoot again: Quit, and the
// window comes straight back. From the moment quitting starts, a fault takes the
// default action and the process stays gone.
static void disarm() {
    struct sigaction dfl {};
    dfl.sa_handler = SIG_DFL;
    for (int sig : {SIGSEGV, SIGBUS, SIGABRT, SIGFPE, SIGILL}) sigaction(sig, &dfl, nullptr);
}
...
int main(int argc, char *argv[]) {
    ...
    QObject::connect(&app, &QCoreApplication::aboutToQuit, [] { crashguard::disarm(); });
    const int rc = app.exec();
    // WOUND DOWN HERE, while every object is still alive, rather than left to the
    // destructors -- which would destroy a job's QThread mid-request and abort.
    // Whatever cannot stop in time is abandoned: its state is Lua's, on its own
    // thread, and everything worth keeping was written before `quit` was sent.
    if (!engine.settle(3000)) {
        fflush(nullptr);
        _exit(rc);
    }
    return rc;
